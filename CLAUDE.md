...
**Étape 5v2 (boutique publique `/[slug]/**`) en cours depuis le 2026-07-21** — plusieurs sessions de resync design (desktop puis mobile) contre une maquette Claude Design fournie par Jake, méthode systématique par mesure DOM/CSS directe via `dev-browser` (jamais à l'œil sur une capture). État au 2026-07-23 : rythme vertical mobile et carrousels horizontaux full-bleed corrigés, header mobile non-sticky, zoom desktop 125% figé en dur (`body:has(.shop-root){zoom:1.25}`, `min-width:1280px`), **nouveau panneau player mobile déplié** (tap sur la barre mini-player → cover 170px/favori/progression/shuffle+loop+prev+play+next/prix — `PlayerContext.tsx` : `isShuffled` mélange persistant, `loopOne` repeat-one, `next()`/`prev()` toujours circulaires y compris en fin de piste naturelle), déclinaison **Blanche & Noire** (thème clair, accent verrouillé noir, `data-accent-preset="blancNoir"`) à jour avec le dernier handoff. Reste hors scope : beat cadeau de fidélité (6.7), CGV/confidentialité/contact/mentions légales/plan de site déjà construits (`app/[slug]/cgv`, `/contact`, etc. — vérifié le 2026-07-24, ne pas les recompter comme manquants).
