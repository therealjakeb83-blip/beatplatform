...
function rendreEmailTransactionnel({
  ...
  const blocFooterSombre = `
      <tr><td style="background:#111827;padding:28px 24px;">
        <table width="100%" cellpadding="0" cellspacing="0"><tr>
          ...
          <td style="vertical-align:middle;padding-left:16px;">
            ${reseaux.length ? `
            <p style="font-size:15px;font-weight:700;color:#ffffff;margin:0 0 4px;">Suis-moi sur les réseaux sociaux</p>
            <p style="font-size:12px;color:#9ca3af;margin:0 0 12px;">Rejoins-moi sur mes réseaux pour rester à jour et me contacter facilement !</p>
            <div>
              ${reseaux.map(r => `
                <a href="${r.lien}" aria-label="${r.label}" style="display:inline-block;width:32px;height:32px;line-height:32px;margin-right:8px;border-radius:50%;background:#ffffff;text-align:center;vertical-align:middle;">${r.svg}</a>`).join('')}
            </div>` : `<p style="font-size:15px;font-weight:700;color:#ffffff;margin:0;">${echapper(branding.nom_artiste)}</p>`}
          </td>
        </tr></table>
