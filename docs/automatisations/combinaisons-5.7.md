...
- **Anti-spam Relance inactivité (2026-07-16)** : l'index unique partiel posé pour le Test 8 (voir `supabase/fix_unicite_evenements_repetes.sql`) résout abonnement_en_attente/churn mais aurait cassé Relance inactivité dans l'autre sens — `reference_id` = dernière commande du client, qui ne change jamais tant qu'il reste inactif, donc un mail traité libérait la contrainte et le scan du lendemain en redéposerait un autre : un envoi par jour à l'infini pour un client qui n'a rien fait de plus. Corrigé dans `scannerRelanceInactivite` (`scans-automatisations/route.ts`) par une vérification explicite (n'importe quel statut traité) avant insertion, indépendante de la contrainte DB — trouvé par Jake en posant la bonne question avant de passer au Test 17, pas en testant en vrai (aurait pris un mois à se manifester).
...
- [x] **9.** Achat + Churn le même jour → seul le remerciement d'achat part. **Validé 2026-07-16.**
...
- [x] **13.** 2 téléchargements gratuits le même jour → 1 mail, 2 titres cités. **Validé 2026-07-16.**
- [x] **14.** Télécharge un gratuit puis achète ce même beat avant l'envoi → aucun mail (garde-fou `achete`, tout nouveau ce soir-là). **Validé 2026-07-16** (bonus : bug titres dupliqués sur re-téléchargement du même beat trouvé et corrigé au passage).
- [x] **15.** Télécharge un gratuit et achète autre chose le même jour → seul le remerciement d'achat part. **Validé 2026-07-16.**

### 7. Relance inactivité
- [x] **16.** Abonné actif qui paie chaque mois sans jamais acheter de licence → ne reçoit **jamais** de mail de relance/code promo (le correctif du 2026-07-14, le plus important à vérifier). **Validé 2026-07-16** (scan lancé manuellement depuis Vercel Cron Jobs → abonnés-sans-achat absents de la file, vrais clients inactifs de 4 mois toujours détectés correctement).
- [x] **17.** Client relancé (événement déposé par le scan) rachète avant l'envoi → aucun mail de relance ne part, remerciement d'achat part à la place. **Validé 2026-07-16.**

**Les 17 tests sont tous validés (2026-07-16).** 4 vrais bugs trouvés et corrigés au passage : Contacts (page liste cassée par la migration Panier + LTV excluant les abonnements), repli combo D+A (2 mails au lieu d'1), index unique trop strict sur `automatisation_evenements` (bloquait les réoccurrences légitimes), Relance inactivité qui aurait spammé quotidiennement, titres de free download dupliqués, recherche par email absente de l'onglet Clients. Plus 1 décision produit révisée (#2, A+C) et 1 nouvelle fonctionnalité client (statut "Paiement en attente" + reprise d'abonnement en self-service).
