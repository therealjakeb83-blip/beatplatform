...
| 2 | A+C — Bienvenue abo + Churn le même jour | Réaliste (abo puis regret immédiat) | Silence total | Silence total — rien de sincère à dire, cohérent avec le principe déjà posé (état net = comme si rien ne s'était passé). |
...
| 4 | D+A — Achat + Bienvenue abo | Réaliste, cas prioritaire cité par Jake | Combo codée en dur | Combo codée en dur — un seul mail qui fusionne remerciement achat + bienvenue abo. |
...
- Il ne reste **aucune combo codée en dur** après révision de #5 : seul D+A (#4) fusionne 2 workflows en un mail. Tout le reste se résout par domination (un signal gagne, l'autre est silencieux) ou par règle fixe (paliers/téléchargements cumulés).
...
- **Combo D+A pas encore configurée par le beatmaker** : comportement par défaut = comme avant 5.7, achat et bienvenue abo partent chacun séparément (aucune régression pendant que la recette combo n'est pas encore activée/enregistrée).
