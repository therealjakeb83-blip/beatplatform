...
export async function POST(
  ...
  const { data: split } = await admin
    .from('beat_splits')
    .select('id, email_invite, beatmaker_id, statut')
    .eq('id', id)
    .single()
  ...
  const estInvite = split.email_invite?.toLowerCase() === userEmail?.toLowerCase()
  const estLie   = split.beatmaker_id === user.id
  if (!estInvite && !estLie) return NextResponse.json({ erreur: 'Non autorisé' }, { status: 403 })
