...
import type { VariableLicence } from '@/lib/licences-textes'
...
export default function LicenceTexteForm({
  ...
  async function enregistrer() {
    ...
    const res = await fetch('/api/licences/textes', {
      ...
      body: JSON.stringify({ licence_id: licenceId, contenu }),
    })
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-6 items-start">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-2">
          {updatedLe
            ...
            : 'Pas encore enregistré — le modèle par défaut est utilisé pour l\'instant sur cette licence.'}
        </p>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 lg:sticky lg:top-6">
        <p className="text-[10px] font-semibold uppercase tracking-widest text-gray-500 mb-3">
          Variables disponibles
        </p>
        <p className="text-xs text-gray-500 mb-4">
          Résolues automatiquement à chaque vente — laisse-les telles quelles dans le texte, ne les remplace pas à la main.
        </p>
        <div className="flex flex-col gap-3">
          {variables.map(v => (
            <div key={v.token}>
              <code className="text-xs text-indigo-400 font-mono block">{v.token}</code>
