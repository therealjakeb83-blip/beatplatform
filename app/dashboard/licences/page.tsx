...
import LicencesManager from './LicencesManager'
...
export default async function LicencesPage() {
  ...
  let { data: licences } = await supabase
    .from('licences')
    .select('*')
  ...
  if (!licences || licences.length === 0) {
    ...
    const { data } = await supabase.from('licences').select('*').eq('beatmaker_id', user.id).order('ordre')
    licences = data ?? []
  }

  return <LicencesManager licences={licences} />
}
