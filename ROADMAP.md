...
| # | Sous-étape | Statut |
|---|-----------|--------|
| 5.1 | Migration SQL (`supabase/phase5_automatisations.sql`) : `automatisations` (config par recette), `automatisation_evenements` (file d'attente déposée par les webhooks, vidée par le cron J+1), `automatisation_envois` (log idempotent) + RLS | ✅ |
| 5.2 | Les 8 workflows en isolation (contenu + déclencheur + délai J+1) — **construits un par un** : 1/8 fait (Bienvenue abonnement, pilote) | 🔄 |
| 5.3 | Cron quotidien `/api/cron/automatisations` (pattern `splits-expiration`) : scan `automatisation_evenements` non traités, vieux d'1 jour | ✅ *(pilote)* |
| 5.4 | Hooks événementiels : `abonnement/succes/route.ts` résout/crée le client puis dépose l'événement `abonnement_bienvenue` | ✅ *(pilote — 1/6 événements)* |
| 5.5 | Page `/dashboard/business/marketing/automatisations/` : éditeur simple (objet/corps + palette de variables), toggle actif — structure prête à accueillir les 7 autres recettes | ✅ *(pilote)* |
