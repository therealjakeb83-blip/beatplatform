...
| 5.6d | Remerciement achat | 4 recettes indépendantes : **1er achat** (validé), **2e** (texte finalisé par Jake), **3e achat**, **4e achat et +** (textes finalisés par Jake — le 4e+ pensé pour rester sincère répété à l'identique de la 4e à la 10e+ commande). Token `{{titre_beats}}` sur les 4 : cite les titres achetés (1-3 noms) ou "les N beats" au-delà. Webhook choisit la recette selon le nombre total de commandes LICENCE du client chez ce beatmaker | 🔄 *(1er ✅ validé T22/T23 ; 2e/3e/4e+ codées et textes finalisés, pas encore testées)* |
| 5.6e | Bienvenue perso | Compte créé sans achat/abo le jour même — ne se déclenche que si rien d'autre ne s'est passé ce jour-là (règle de suppression vérifiée à l'envoi, pas au dépôt). Codé le 2026-07-10 : hook dans `lierCompteClient` (inscription et connexion — 1re fois que ce client est connu de CE beatmaker) | 🔄 *(codé, pas encore testé)* |
| 5.6f | Relance inactivité | X mois sans achat, configurable par le beatmaker (`config.mois_inactivite`, défaut 3). N'existe pas chez Jake, construit de zéro : scan cron (pas de déclencheur ponctuel), `reference_id` = dernière commande LICENCE du client, se redéclenche naturellement après un nouvel achat suivi d'une nouvelle inactivité. **Ajout 2026-07-10 :** code promo personnel généré automatiquement à l'envoi (`code_promo`, `pourcentage_remise` — 2e paramètre configurable, défaut 50%, `date_expiration_code` — 30 jours), réservé à l'email du client, usage unique — voir note ci-dessous | 🔄 *(codé, pas encore testé)* |
| 5.6g | Follow-up free download | Tracking existant (`free_downloads`) — hook direct dans `/api/free-download`. Sauté à l'envoi si le beat a été acheté entre-temps. Texte rédigé par Claude (pas de référence Jake), à valider | 🔄 *(codé, pas encore testé)* |
| 5.6h | Follow-up favori | Tracking existant (`favoris`, jointure `beats` pour `beatmaker_id`) — pas de route serveur (insert client-side), donc scan cron plutôt que hook direct. Fenêtre de 5 jours, sauté à l'envoi si le beat a été acheté entre-temps. Texte rédigé par Claude (pas de référence Jake), à valider | 🔄 *(codé, pas encore testé)* |
...
| 5.2 | Les 8 workflows en isolation (contenu + déclencheur) — **construits un par un** : les 8 sont codés (2026-07-10). Validés en test réel : Bienvenue abonnement, Abonnement en attente, Churn message perso, Remerciement achat 1er achat. Codés mais pas encore testés : Remerciement 2e/3e/4e+, Bienvenue perso, Relance inactivité, Follow-up free download, Follow-up favori | 🔄 |
...
>
> **Les 8 workflows tous codés le 2026-07-10 (Jake ne pouvait pas tester dans l'immédiat, a demandé de tout coder d'un coup) — 4 restants (5.6e-h), jamais testés à ce stade :**
...
> - **Relance inactivité** : premier paramètre configurable par le beatmaker (mois d'inactivité, via la colonne `config` jsonb déjà prévue depuis 5.1) — nouveau champ générique `champConfig` sur `Recette`, rendu par `RecetteCard`. Pas de déclencheur ponctuel (c'est un état, pas un événement) → scan cron quotidien : dernière commande LICENCE par client, relance si plus vieille que le seuil configuré. `reference_id` = cette dernière commande, donc un nouvel achat suivi d'une nouvelle inactivité redéclenche naturellement (nouvelle commande = nouveau `reference_id`).
> - **Follow-up free download** : hook direct dans `/api/free-download/route.ts` (route serveur existante). Sauté à l'envoi si `free_downloads.achete` est passé à `true` entre-temps.
> - **Follow-up favori** : `favoris` est inséré **côté client** (navigateur, via Supabase direct) — pas de route serveur à hooker sans changer l'UX existante du bouton cœur. Scan cron à la place (fenêtre de 5 jours). Sauté à l'envoi si le client a acheté ce beat entre-temps (jointure `commande_lignes`→`commandes`).
> - **Nouveau cron `/api/cron/scans-automatisations`** (regroupe Relance inactivité + Follow-up favori dans une seule route plutôt que d'ajouter 2 entrées à `vercel.json` — Jake est peut-être encore sur le plan Vercel Hobby, à vérifier avant que le nombre de crons ne pose problème au déploiement).
...
> - Follow-up free download et Follow-up favori n'ont **aucun texte de référence de Jake** (contrairement aux 6 autres workflows) — rédigés par Claude dans le ton observé sur les autres textes, **à relire et ajuster par Jake**, pas juste à tester techniquement.
>
> **5.6f — code promo personnel dans Relance inactivité (2026-07-10) :** Jake veut un vrai levier de conversion, pas juste un message ("je te fais un geste... -{{pourcentage_remise}}% avec le code {{code_promo}}"), ton toujours amical, pas commercial/agressif. Le code est créé dans `codes_promo` **uniquement à l'envoi réel** (pas à la prévisualisation, pour ne pas générer de codes fantômes à chaque clic sur "Visualiser") : `type_remise: 'panier'`, `emails_autorises: [email du client]` (personnel, pas partageable), `limite_par_utilisateur: 1` (usage unique), expire 30 jours après l'envoi (décision Jake : crée un peu d'urgence). Pas d'appel Stripe (Coupon/PromotionCode) — comme tous les codes `panier`/`produit` existants, la réduction est calculée côté serveur au checkout, jamais transmise à Stripe. `pourcentage_remise` devient le 2e paramètre configurable de cette recette (`champsConfig`, plus seulement `champConfig` — `RecetteCard` gère maintenant un tableau de champs numériques).
