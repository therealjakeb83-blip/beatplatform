...
> - **Statut actuel : Phase 1 (pages légales réelles) en cours**, démarrée le 2026-08-08 en parallèle de cette mise à jour de roadmap. Premier artefact technique : `supabase/pages_legales.sql` (table `boutique_pages_legales` — une ligne par `beatmaker_id`/`type_page` parmi cgv/mentions_legales/confidentialite/contact/plan_de_site, versionnée, RLS par beatmaker ; sans ligne, le template par défaut de `lib/pages-legales.ts` s'affiche). Distincte de `beatmakers.cgv_acceptees_at`, qui concerne l'acceptation des CGV de My Producer par le beatmaker (relation SaaS), pas les CGV que le beatmaker publie vers ses propres clients. Reste les phases 2 à 12 à cadrer/coder, **Phase 13 volontairement gelée** en attendant la réponse du dossier partenaire PayPal.
