...
>
> **Phase 5 démarrée** : audit d'impact avant tout code (grep systématique des requêtes `commandes.beats(...)`/`commandes.licences(...)` — 3e occurrence du même bug de relation orpheline post-Phase 2c, cette fois sur le bouton "Renvoyer par email", cassé depuis le 09/07, corrigé et validé). Lot 1-2 codés et validés : nouveau `commandes.statut_livraison` (en_cours/livree/probleme), calculé depuis l'état réel des contrats PDF (`commande_lignes.contrat_pdf_url`) et des transferts collab (`split_payments.statut`, en distinguant un vrai échec de transfert d'une attente légitime — collab pas encore inscrit) — remplace le booléen `fichiers_livres` qui restait bloqué à `false` pour toujours dès qu'une seule opération échouait, sans jamais rien retenter. Reste à faire : affichage sur la fiche commande, bouton de reprise par opération individuelle, alerte email au beatmaker.
>
