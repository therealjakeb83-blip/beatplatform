...
> Dernière mise à jour : 2026-07-27 — **Étape 15 lot 2 (Admin) : 15d Analytics plateforme codé (RIEN N'EST ENCORE TESTÉ PAR JAKE), 15e Mails transactionnels reconsidéré et simplifié après discussion.**
...
> - Checklist de test T0-T14 pour 15d ajoutée plus bas (section "Checklist tests Étape 15 lot 2").
>
...
| 5 | **Étape 15 lot 2** — Admin : 15d Analytics plateforme ✅ *(codé le 2026-07-27, pas encore testé)*, 15e simplifié (voir note) | Repoussé après l'Étape 8b (rang 4) : 15d mesure justement le revenu de l'Étape 8b — le construire avant afficherait des zéros indéfiniment. |
...
| 15 | **Admin** | Back-office plateforme, périmètre cadré en sous-étapes le 2026-07-24 : **15a** Recherche/Support ✅ codé+testé, **15b** Log Stripe ✅ codé+testé, **15c** Suspendre/Réactiver une boutique (cascade Stripe) ✅ codé+testé (3 bugs trouvés et corrigés pendant les tests), **15d** Analytics plateforme ✅ *(codé le 2026-07-27, pas encore testé)*, **15e** Visibilité mails transactionnels 🔄 *(liste détaillée toutes boutiques codée puis retirée le 2026-07-27 — redondante avec les logs déjà propres à chaque beatmaker + trop d'exposition sur les données de leurs clients ; ne reste que le compteur agrégé dans 15d. Le vrai besoin de Jake identifié après coup : "Mails My Producer", à cadrer séparément)*, **15f** Rôles/permissions (reste en V1, gate par slug) — pas prioritaire, **15g** Mises à jour/veille technique ⬜ *(nouvelle zone née le 2026-07-24)*, modération des demandes de certification (catégories, 11d Phase 7) ✅ *(amorcé 2026-07-17, `/dashboard/admin/categories`)*, outil interne d'import BeatStars (script scraping concierge) ⬜. Détail complet + scoring dangerosité : mémoire `project_admin_etape15_scope`. Dépendance dure avant l'étape 17 (déploiement officiel) | 10-15h *(à réviser)* | 🔄 En cours — lot 1 (15a/15b/15c) validé le 2026-07-24, 15d codé le 2026-07-27 (tests en attente), 15e simplifié |
...
#### Checklist tests Étape 15 lot 2 — Admin 15d Analytics plateforme ⬜ Pas encore testée par Jake
...
**Accueil admin :**
- [ ] **T0** — `/dashboard/admin` affiche la nouvelle carte "Analytics plateforme", lien fonctionnel
...
- [ ] **T13** — Bloc "Santé technique" : le compteur d'échecs email (7j) correspond au nombre réel de lignes `echoue` dans `email_logs` sur la période ; le compteur webhook + son lien vers `/dashboard/admin/stripe-events?filtre=echoue` fonctionnent
- [ ] **T14** — Bloc "Tendances catalogue" : total de beats cohérent avec la somme des catalogues de toutes les boutiques

**Sécurité :**
- [ ] **T15** — Compte non-admin (déconnecté ou autre compte réel) → `/dashboard/admin/analytics` redirige vers `/dashboard/business`
