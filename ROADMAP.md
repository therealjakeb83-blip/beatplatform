...
> Dernière mise à jour : 2026-07-14 — **Phase 5 (Marketing : Automatisations) validée de bout en bout — les 7 workflows testés en conditions réelles** : Bienvenue abonnement, Abonnement en attente, Churn message perso, Remerciement achat (4 paliers, y compris le redéclenchement du 4e+ à la 5e commande), Relance inactivité (avec génération automatique d'un code promo personnel), Bienvenue perso (inscription + connexion sur compte existant), Follow-up free download. Follow-up favori codé puis retiré (jugé trop intrusif par Jake). **2 bugs Commerce préexistants trouvés et corrigés en testant le code promo de Relance inactivité** : validation email d'un code restreint qui l'affichait "appliqué" avant vérification (fail closed maintenant), et l'exclusion des licences illimité/exclusive qui bloquait à tort les codes promo en plus de la remise abonné automatique. **Bonus** : toggle actif d'une recette qui s'enregistre désormais immédiatement (au lieu d'attendre un clic sur "Enregistrer"), page Automatisations réorganisée en catégories/sous-pages, boutons Visualiser/Supprimer sur la file d'attente. Reste avant de clore la Phase 5 : 5.7 (règles de combinaison entre workflows, dont la suppression de Bienvenue perso reportée depuis le 2026-07-14) et 5.8 (IA + validation humaine pour les cas rares).
...
| 5.6g | Follow-up free download | Tracking existant (`free_downloads`) — hook direct dans `/api/free-download`. Sauté à l'envoi si le beat a été acheté entre-temps. Texte rédigé par Claude (pas de référence Jake), finalisé par Jake | ✅ *(validé le 2026-07-14 : cas simple + cas suppression)* |
...
| 5.7 | Combinaisons : règles de résolution (état net, slots, templates combinés fréquents). **À réintégrer ici** : la règle de suppression de Bienvenue perso (rien d'autre le même jour — voir 5.6e), codée le 2026-07-10 puis retirée le 2026-07-14 car prématurée pendant la phase d'isolation | ⬜ |
