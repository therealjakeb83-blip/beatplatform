...
| 5.6d | Remerciement achat | Scindé en 2 automations distinctes (décision Jake, 2026-07-09) : **1er achat** codé et validé (token `{{titre_beats}}` cite les titres réellement achetés, pas "le beat"/"les beats"), **paliers client récurrent** (2e/3e/4e et +, basé sur données réelles : 1.5 commande/client en moyenne, 3.1 chez les récurrents) reporté à une session dédiée — voir note ci-dessous | 🔄 *(1er achat ✅ validé le 2026-07-10, paliers récurrents restants)* |
