...
| 5.6d | Remerciement achat | 4 recettes indépendantes : **1er achat** (validé), **2e/3e/4e achat et +** (codées le 2026-07-10, texte par défaut = texte "récurrent" verbatim de Jake pour les 3, éditables séparément). Token `{{titre_beats}}` sur les 4 : cite les titres achetés (1-3 noms) ou "les N beats" au-delà (ajusté le 2026-07-10, citer+"et N autres" sonnait robotique). Webhook choisit la recette selon le nombre total de commandes LICENCE du client chez ce beatmaker | 🔄 *(1er achat ✅ validé ; 2e/3e/4e+ codées, migration à exécuter + à tester)* |
