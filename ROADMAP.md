...
> Dernière mise à jour : 2026-08-08/11 — **Nouveau chantier ouvert, hors refonte UX/UI : revue juridique/fiscale de toute l'architecture de paiement (risque « fournisseur réputé », article 9 bis du règlement TVA UE 282/2011) — plan de refonte en 13 phases, Phase 1 démarrée.**
> - **Contexte** : si My Producer est qualifiée de fournisseur réputé, la plateforme devrait collecter 20% de TVA sur **toutes** les ventes de **tous** les beatmakers, y compris les micro-entrepreneurs — un scénario déjà observé sur BeatStars par le passé. Un audit de l'architecture actuelle (paiements Stripe, fulfillment, licences, splits, CGV, facturation) a mis en évidence plusieurs signaux qui rapprochent la plateforme de ce statut : ventes qui passent par une "destination charge" Stripe (My Producer techniquement partie prenante du paiement, pas juste le beatmaker), CGV/mentions légales encore vides sur toutes les boutiques ("Contenu à compléter"), bouton "Rembourser" qui ne fait qu'un flag en base sans jamais appeler Stripe, clauses de licence figées dans le code (identiques pour tous les beatmakers, jamais éditables), et aucune facture jamais générée. Un bug réel trouvé en creusant (sans lien direct avec le 9 bis) : le reversement des fonds collaboratifs est resté silencieusement bloqué 3 semaines (09/07-29/07/2026).
> - **Déroulé de la session** : (1) audit initial de l'architecture de paiement/fulfillment ; (2) analyse technique d'une architecture "multi-Direct-Charge" Stripe pour les ventes collaboratives (coûts réels identifiés : authentification bancaire potentiellement dupliquée, aucune garantie de transaction atomique, frais croissants avec le nombre de collaborateurs) ; (3) comparatif documentaire Stripe vs PayPal Multiparty/Multi-Seller pour ces mêmes ventes collaboratives (PayPal mieux disant sur la détention des fonds et la détection d'échec partiel, mais nécessite un statut de partenaire approuvé PayPal non garanti pour la France/UE) ; (4) "Grill Me" interactif — 19 objections classées critique/important/à cadrer, tranchées une par une avec Jake, plusieurs décisions initiales corrigées en route ; (5) plan d'implémentation en 13 phases, audité en retour par Jake lui-même (9 corrections techniques intégrées), puis découpé en tâches atomiques phase par phase. Documents produits (non archivés dans ce dépôt vu leur taille) : Audit article 9 bis, Multi-Direct-Charge (splits), Stripe vs PayPal Multiparty, Grill Me — synthèse finale, Plan d'implémentation, Découpage en tâches atomiques.
> - **Architecture cible retenue** : principe général **"le beatmaker décide, My Producer exécute techniquement"**. Ventes simples → bascule vers une vraie "Direct Charge" Stripe (paiement créé directement sur le compte Stripe du beatmaker, la plateforme ne fait qu'exécuter). Ventes collaboratives → le choix du processeur (PayPal Multiparty ou repli Stripe) est **volontairement gelé en toute dernière phase**, le temps d'obtenir la réponse du dossier partenaire PayPal ; ça ne bloque aucune des 12 autres phases. Le reste de l'architecture s'appuie sur un mandat de fulfillment explicite, un snapshot transactionnel immuable par commande, un mandat commercial entre collaborateurs, des licences et CGV librement éditables par chaque beatmaker, et un nouveau journal des décisions commerciales (`merchant_decisions_log`).
> - **Les 13 phases** : 1. Pages légales réelles (🔄 en cours) — 2. Direct Charge + onboarding beatmaker (livraison/moyens de paiement/identité sur relevé bancaire) — 3. Remboursement Stripe réel (aujourd'hui un trou fonctionnel pur) — 4. Snapshot transactionnel par commande — 5. Supervision technique + statut de commande fiable + reprise admin encadrée — 6. Licences éditables par le beatmaker — 7. Journal des décisions commerciales — 8. Facturation réelle + mandat de facturation — 9. Réglage TVA ajoutée/absorbée + fondations extension internationale (sans moteur fiscal complet) — 10. Suspension de boutique à motif fermé + traçabilité — 11. Téléchargement post-achat sécurisé (aujourd'hui un simple lien difficile à deviner) — 12. Modèle de collaboration complet entre beatmakers (données/invitation/acceptation/mandat/éviction, sans la mécanique de paiement) — 13. Processeur de paiement des collaborations (gelé, dépend de PayPal).
> - **Backlog noté 2026-08-28, pas cadré** : Jake veut une refonte complète des licences (pas juste "éditables", tout le contenu/structure retravaillé) — même principe que les CGV, un modèle par défaut proposé par My Producer que le beatmaker peut exploiter tel quel ou modifier. Les licences actuelles n'ont jamais été vraiment travaillées de fond. À rapprocher de la Phase 6 au moment de la cadrer — voir `memory/project_features_backlog.md`.
> - **Backlog noté 2026-08-28** : tous les emails **plateforme → beatmaker** (alertes techniques, notifications d'abonnement — pas les emails boutique → client déjà travaillés en Phase 6) seront retravaillés texte + design avant le lancement, à rapprocher de la refonte de la page de validation de commande notée juste au-dessus.
> - **Chantier séparé cadré le 2026-08-28 (pas une phase de celui-ci)** : réglage de fuseau horaire par beatmaker, prévu juste après ce chantier juridique/fiscal dans l'ordre de traitement général — découvert via un bug d'affichage (dates -2h sur les pages calculées côté serveur), estimé 4-7h. Deux volets : affichage (11 pages Server Component identifiées) et, plus important, le découpage des périodes dans les Analytics (calcul de "ce mois-ci" etc. fait en UTC côté serveur au lieu du fuseau du beatmaker — un vrai problème de regroupement des données, pas juste d'affichage). Détail complet : `memory/project_fuseau_horaire_par_beatmaker.md`.
> - **Statut actuel (2026-08-28) : Phases 1 à 5 entièrement terminées et validées (T5/T6 d'immuabilité de la Phase 4 exécutés — voir entrée dédiée plus bas ; Phase 5 close en 5 lots — statut/affichage/reprise/alerte). Une partie de la Phase 9 (TVA toujours absorbée) traitée par anticipation le même jour — voir entrée dédiée.** Phase 1 : `supabase/pages_legales.sql` (table `boutique_pages_legales` — une ligne par `beatmaker_id`/`type_page` parmi cgv/mentions_legales/confidentialite/contact/plan_de_site, versionnée, RLS par beatmaker ; sans ligne, le template par défaut de `lib/pages-legales.ts` s'affiche). Distincte de `beatmakers.cgv_acceptees_at`, qui concerne l'acceptation des CGV de My Producer par le beatmaker (relation SaaS), pas les CGV que le beatmaker publie vers ses propres clients. Phase 2 : bascule complète vers Direct Charge, toutes les boutiques — voir entrée détaillée du 2026-08-27 plus bas. Phase 3 : remboursement Stripe réel pour les ventes solo Direct Charge, ventes collab volontairement bloquées — voir entrée détaillée du 2026-08-28 plus bas. Reste les phases 6 à 12 à cadrer/coder, **Phase 13 volontairement gelée** en attendant la réponse du dossier partenaire PayPal. Chantier séparé (fuseau horaire, hors plan) cadré mais pas codé.
> - **Prudence de formulation** : ce chantier réduit le risque et clarifie la séparation des rôles (le beatmaker reste vendeur, My Producer exécute) — il ne s'agit à aucun moment d'une garantie juridique que l'article 9 bis ne s'appliquera pas.
>
> Dernière mise à jour : 2026-08-27 — **Phase 2 (Direct Charge) entièrement terminée et validée — audit d'impact préalable, bascule complète de toutes les boutiques, 3 vrais bugs Stripe trouvés et corrigés en testant.** Avant tout code : audit dédié de la chaîne d'impact (demandé par Jake pour une phase à risque de régression élevé) qui a révélé 2 points absents du découpage initial — le client Stripe.js était un singleton global incompatible avec Direct Charge, et un bug latent pré-existant dans la pause d'abonnement (signalé, hors scope, pas corrigé). Implémentation en incréments testés un par un (2.1 mandat de fulfillment, 2.2 moyens de paiement, 2.3 identité sur le relevé bancaire, 2.4/2.4b bascule checkout + client Stripe.js par compte, 2.6/2.7 webhook Connect dédié, 2.8 snapshot `stripe_account_id`/`payment_intent_id` sur `commandes`, 2.9 nettoyage `devise`). **3 bugs réels trouvés en testant un premier achat Direct Charge sur `jakeb-test`**, aucun anticipé par l'audit initial : (1) PayPal rejeté par Stripe sur toute charge Connect (Direct Charge ou `on_behalf_of`), y compris en paiement express — retiré du réglage moyens de paiement, comme "virement" avant lui (aucun cas d'usage identifié, décision de Jake) ; (2) Apple Pay affiché mais bloqué indéfiniment en "Traitement en cours" — mauvaise API d'enregistrement de domaine utilisée (`ApplePayDomain`, historique) au lieu de la bonne (`PaymentMethodDomain`, unifiée, exigée par Express Checkout Element pour un compte connecté — trouvé en lisant la doc Stripe après un diagnostic long sur iPhone sans accès Mac, `alert()` silencieusement bloqué par Safari en contexte async, remplacé par un panneau de log visible à l'écran) ; (3) email acheteur jamais capturé sur un achat express anonyme (`receipt_email` vide, l'email collecté par le wallet lui-même n'était jamais lu) — préexistant à Direct Charge, jamais remarqué faute de test avec un acheteur vraiment anonyme. **Décision de Jake, argumentée et retenue** : une fois Direct Charge validé, bascule immédiate de toutes les boutiques (pas de rollout progressif) et suppression complète de l'ancien flux destination charge (route `on-behalf-of`, flag `direct_charge_actif`) plutôt que de le garder comme filet de secours — raisonnement : il n'y avait pas de filet quand destination charge était le seul flux, pas de raison d'en garder un maintenant. `tsc`/`eslint` propres à chaque étape, chaque commit testé par Jake avant le suivant. Détail complet : `memory/project_phase2_direct_charge_implementation.md`.
>
> Dernière mise à jour : 2026-08-28 — **Phase 3 (remboursement Stripe réel) terminée et validée, scopée aux ventes solo Direct Charge.** Audit d'impact préalable : le bouton "Rembourser" ne faisait qu'un `UPDATE commandes SET statut='remboursee'`, aucun appel Stripe nulle part. 3 cas identifiés selon `stripe_account_id`/`stripe_transfer_group` : (A) Direct Charge solo → codé, `stripe.refunds.create` sur le compte connecté figé à la vente ; (B) ancien destination charge solo (avant bascule du 27/08) → volontairement pas codé, aucune nouvelle commande ne peut plus tomber dans ce cas (bascule totale), et les données de test existantes seront effacées au lancement (voir `memory/feedback_pas_encore_lance_pas_de_panique.md`, renforcée cette session) — erreur explicite si jamais cliqué ; (C) vente avec collaborateur(s) (`stripe_transfer_group` rempli) → bouton remplacé par un badge, aucun appel Stripe tenté, car l'argent est déjà distribué en `Transfer` séparés à chaque collaborateur à la vente et un simple refund ne récupère rien chez eux (clawback proportionnel gelé jusqu'au choix du processeur collab, Phase 13). Décision de Jake actée : la gouvernance du remboursement collab (qui autorise, comment l'argent revient) sera retranchée avec ce choix de processeur — 2 pistes notées dans `memory/project_grillme_9bis_synthese.md`. Testé en conditions réelles : commande Direct Charge existante sur `jakeb-test` remboursée via le bouton, vérifié directement via l'API Stripe (`stripe.refunds.list`) — remboursement `succeeded`, montant complet, sur le bon compte connecté. `tsc`/`eslint` propres. Détail complet : `memory/project_grillme_9bis_synthese.md`.
>
> Dernière mise à jour : 2026-08-28 — **Phase 4 (snapshot transactionnel) terminée et validée, scopée à ce qui existe déjà dans le code.** Périmètre : TVA (vrai taux du beatmaker capturé par vente, corrige au passage un 20% codé en dur dans l'affichage de la fiche commande), version des CGV et du mandat de fulfillment en vigueur, licence achetée (nom + modèle + fichiers inclus) — figés indépendamment d'une modification future (licences éditables = Phase 6, pas encore codée). Fichiers audio eux-mêmes volontairement exclus (Jake : remplacer un mauvais upload doit rester possible sans lien avec les ventes déjà passées). Prérequis découvert et corrigé en route : `boutique_pages_legales` (Phase 1) écrasait le texte d'une ancienne version à chaque modification — nouvelle table `boutique_pages_legales_historique`, qui archive chaque version remplacée pour toutes les pages légales. **Bug réel trouvé en testant (T0)** : la politique RLS d'insertion sur cette table d'historique manquait (seul un `GRANT` existait, insuffisant avec RLS activé) — l'archivage échouait silencieusement, corrigé par `supabase/phase4_fix_rls_historique.sql`. Ajout demandé par Jake, calqué sur la pratique des grosses plateformes : date "Dernière mise à jour le [date]" sur les pages CGV/mentions légales/confidentialité publiques. Checklist T0-T3 100% validée. **Reste à faire avant de considérer la phase vraiment bouclée** : vérifier l'immuabilité dans le temps (T5/T6, jamais exécutés) — modifier les CGV/la TVA après une vente déjà passée et confirmer que `commandes.cgv_version`/`tva_taux` de cette vente ne bougent pas. `tsc`/`eslint` propres. Détail complet : `memory/project_phase4_snapshot_implementation.md`.
>
> Dernière mise à jour : 2026-08-28 (incident) — **Panne Supabase (API Gateway dégradée, incident confirmé sur status.supabase.com) a révélé un bug de fiabilité ancien dans `proxy.ts` : aucune limite de temps sur `supabase.auth.getUser()`, exécuté avant absolument chaque page du site.** Résultat vécu en conditions réelles : site entier bloqué pendant les 5 minutes du timeout Vercel pour les sessions concernées. Diagnostic mené via `curl` (élimine panne globale), reproduction du parcours de connexion réel avec un compte de test (élimine un bug du code), **`vercel logs`/`vercel inspect --logs`** (CLI déjà authentifié dans cet environnement — a révélé les vraies erreurs `Vercel Runtime Timeout Error: Task timed out after 300 seconds`), puis `status.supabase.com` (confirmé l'incident fournisseur). Preuve que le bug est indépendant du déploiement du jour : `git diff`/`git log` sur `proxy.ts` (non modifié depuis plusieurs sessions), plus solide et plus rapide qu'un rollback réel proposé par Jake. Correctif : chrono de 5 secondes (`Promise.race`) sur `getUser()`, avec distinction volontaire entre "pas connecté" (→ `/connexion`, normal) et "impossible à vérifier à temps" (→ nouvelle page `/verification-en-cours`, jamais le formulaire de connexion, pour ne pas laisser croire à une déconnexion). Texte de la page retravaillé avec Jake (ne jamais promettre une résolution rapide non garantie). Mécanisme vérifié par un test isolé (Promise.race simulée, coupe bien à ~5000ms même avec une réponse à 30s). Détail complet : `memory/project_incident_supabase_timeout_proxy.md`.
>
> Dernière mise à jour : 2026-08-28 — **Estimations de temps par phase du chantier 9 bis établies avec Jake** (ordres de grandeur, pas des engagements) : Phase 5 revue à la hausse (6-12h au lieu de 4-8h) — les phases qui auditent des mécanismes déjà existants (crons, webhooks) ont systématiquement fait remonter un bug dormant non cherché sur ce chantier (bug RLS aujourd'hui, timeout `proxy.ts`, cron splits bloqué 3 semaines en juillet), et Phase 5 a exactement ce profil. Détail complet (tableau par phase) : `memory/project_grillme_9bis_synthese.md`.
>
> Dernière mise à jour : 2026-08-28 (suite) — **Phase 4 entièrement close (T5/T6 exécutés) : republication des CGV puis changement de TVA sur une commande de test déjà passée, aucune des deux ne l'a affectée** — l'immuabilité tient dans le temps, pas seulement à la capture. Trou identifié en vérifiant : `cgv_version`/`mandat_fulfillment_version` sont capturés en base mais **jamais affichés dans le dashboard business** — Jake n'a pu vérifier que via une requête directe (script Node ad hoc, service role key déjà dans `.env.local`). Noté pour plus tard, pas cadré.
>
> **Phase 5 entièrement terminée et validée (5 lots)** : audit d'impact avant tout code (grep systématique des requêtes `commandes.beats(...)`/`commandes.licences(...)` — 3e occurrence du même bug de relation orpheline post-Phase 2c, cette fois sur le bouton "Renvoyer par email", cassé depuis le 09/07, corrigé et validé). Nouveau `commandes.statut_livraison` (en_cours/livree/probleme), calculé depuis l'état réel des contrats PDF (`commande_lignes.contrat_pdf_url`) et des transferts collab (`split_payments.statut`, en distinguant un vrai échec de transfert d'une attente légitime — collab pas encore inscrit) — remplace le booléen `fichiers_livres` qui restait bloqué à `false` pour toujours dès qu'une seule opération échouait, sans jamais rien retenter. Backfill nécessaire après coup : l'`ALTER TABLE ... DEFAULT` a mis "en_cours" sur toutes les commandes historiques (comportement Postgres normal), corrigé d'après `fichiers_livres`. Affiché sur la fiche commande business (badge + détail précis de ce qui a raté). Bouton "Réessayer" (`/api/business/commandes/[id]/reprendre-livraison`) qui ne rejoue que l'opération en échec (contrat PDF ou transfert), jamais toute la commande — testé en conditions réelles sur une commande volontairement cassée. Email d'alerte au **beatmaker** (pas l'admin) si un problème persiste après la première tentative, testé et confirmé sans faux positif sur un achat réel. Détail complet : `memory/project_phase5_supervision_livraison.md`.
>
> **Détour Phase 9 (TVA) traité en avance** : en testant, un vrai bug trouvé — le paiement express (Apple Pay/Google Pay) affichait un montant dans la fenêtre native différent de celui réellement facturé (TVA absente du calcul côté client). En creusant la correction avec Jake, décision produit prise en direct : **la TVA est désormais toujours absorbée, jamais ajoutée par-dessus** — le prix tapé par le beatmaker (licence ou abonnement) reste le prix final payé par le client, à vie pour un abonné déjà engagé même si le beatmaker change son réglage TVA ensuite (même principe que le snapshot CGV de la Phase 4). `lib/pricing.ts` ne recalcule plus rien à la hausse ; `abonnements_boutique.tva_taux` fige le taux à la souscription pour les déclarations, jamais recalculé aux renouvellements. Mention "TTC · dont TVA (X%) : Y€" affichée aux points d'achat réels (popup licence, panier, page abonnement/membres) — pas sur les grilles de beats. **Backlog noté** : la page de paiement des abonnements reste la page Stripe standard (hors de notre contrôle, la mention TVA n'y apparaît pas de façon fiable) — Jake prévoit une **refonte complète en page de validation de commande custom avant le lancement**, qui réglera ce point nativement ; pas la peine de creuser davantage la page Stripe standard d'ici là. Détail complet : `memory/project_phase5_supervision_livraison.md` et `memory/project_phase9_tva_absorbee.md`.
>
> Dernière mise à jour : 2026-08-08 — **Fiche produit boutique (Étape 5v2, rang 6) : gabarit desktop livré depuis la maquette `integration-fiche-produit-pc` de Jake, complète la version mobile déjà livrée le 2026-08-06.** Carte deux colonnes (cover 360px + colonne infos) à partir de 1280px (même seuil que le zoom desktop 125% déjà en place), toutes les valeurs reprises de la maquette et confirmées identiques par comparaison directe `getComputedStyle` avec le lien de preview fourni par Jake (pas juste à l'œil sur une capture). Nouveau bouton "Partager" (copie le lien + toast "Lien du beat copié") ajouté côté desktop uniquement, absent du mobile pour l'instant. Un bug de structure trouvé et corrigé en testant : le DOM mobile étant "à plat" (6 enfants directs de la carte), passer en `display:grid` dispersait les éléments façon zig-zag entre les 2 colonnes au lieu de les grouper — corrigé en regroupant tags/infos/CTA dans un wrapper transparent en mobile (`display:contents`) qui devient la vraie 2ᵉ colonne à 1280px. `tsc`/`eslint` propres. **Pas encore re-testé par Jake dans son navigateur.** Détail complet : `memory/project_fiche_produit_desktop.md`.
>
> **À tester par Jake** : ouvrir une fiche produit en desktop (fenêtre large, ≥1280px) sur une boutique de test — vérifier que la carte s'affiche bien en 2 colonnes (cover à gauche, infos à droite), que le bouton "Partager" (icône à côté du cœur) copie bien le lien et affiche le toast, et que le mobile n'a pas changé visuellement.
>
> Dernière mise à jour : 2026-08-06 — **Bug corrigé : PayPal apparaissait puis disparaissait aussitôt dans le paiement express (popup licence ET panier).** Cause : PayPal n'accepte pas du tout le paramètre `on_behalf_of` côté Stripe (contrairement à Apple/Google Pay) — on le résolvait après le montage d'`Elements` via `elements.update()`, ce qui faisait retirer PayPal silencieusement du bouton une fois pris en compte, sans le signaler à nos callbacks React. Corrigé en résolvant `on_behalf_of` **avant** de monter `Elements` (`LicenceExpressPay.tsx`/`CartExpressPay.tsx`), passé dans ses options initiales — la détection des wallets se fait déjà dans le bon contexte Connect dès le départ, plus de flicker. `tsc`/`eslint` propres, smoke-test local (modal + panier s'ouvrent normalement, aucune erreur console). Détail complet : mémoire `feedback_express_payment_pitfalls.md` (piège n°5). **Poussé, pas encore re-testé par Jake sur appareil réel.**
>
> Dernière mise à jour : 2026-08-05 (suite) — **Réductions par lot ("achète X, obtiens Y offert") livrées de bout en bout + refonte complète de la popup panier + paiement express étendu au panier entier — session longue avec 5 bugs réels trouvés et corrigés en testant avec Jake.**
> - **Réductions par lot (nouvelle fonctionnalité)** : table `reductions_lot` (`supabase/reductions_lot.sql`) — une règle = une licence, une seule règle active à la fois par licence (contrainte unique partielle en base, pas juste côté UI). Logique de calcul entièrement dans `lib/pricing.ts` (`calculerLignesPanier`, 3 passes : remise abonné → détection lot/article gratuit → code promo + TVA) — article offert = le moins cher parmi les éligibles, dernier ajouté au panier en cas d'égalité (décision produit de Jake). Cumul avec un code promo configurable **par code promo** (`codes_promo.cumulable_reduction_lot`, nouveau toggle dans la modale existante), pas une règle globale. Dashboard `/dashboard/business/reductions-lot` (CRUD, aperçu en direct). **Activée par défaut sur toutes les boutiques** (existantes en backfill non destructif + nouvelles via le trigger `creer_licences_defaut()` étendu) : "achète 2, obtiens 1 offert" sur toutes les licences sauf Exclusive. **Testé bout en bout par Jake** (panier réel → Stripe → montant correct, TVA appliquée par-dessus, tie-break "dernier ajouté" confirmé).
> - **Noms de licences par défaut simplifiés** : MP3/WAV/STEMS/ILLIMITÉ/EXCLUSIVE (au lieu de "MP3 Basic"/"MP3 + WAV"/etc.) — backfill non destructif (ne touche jamais une licence déjà renommée par un beatmaker) + trigger et fallback JS mis à jour pour les nouvelles boutiques.
> - **Refonte popup panier** (`CartDrawer.tsx`, mobile + PC) d'après un handoff Claude Design (`handoff_panier_v1`) : popup **blanche** volontaire (contraste avec le reste du site sombre), pixel-perfect vérifié contre la maquette live via `dev-browser` (mesures `getBoundingClientRect`/styles inline réels, pas à l'œil) — panneau ancré à droite en PC, modale quasi plein écran arrondie des 4 côtés en mobile (pas un bottom-sheet). Bandeau promo + article "GRATUIT" affichés **en temps réel dans le panier** (avant même de cliquer "Passer commande"), pilotés par une logique client miroir de `lib/pricing.ts` (`app/[slug]/_lib/reductions-lot.ts`, affichage uniquement — le montant facturé reste toujours recalculé serveur). `--ac-deep` (formule `oklch(from...)`) utilisé uniquement en valeur directe dans le nouveau CSS, jamais imbriqué dans `color-mix()` (cf. bug documenté sur le preset noir&blanc).
> - **Paiement express étendu au panier multi-articles** (décision de Jake : dupliquer exactement la logique déjà validée du sélecteur de licence) : `/api/stripe/express-checkout` généralisé pour accepter `items[]` en plus de `beat_id`/`licence_id` (rétrocompatible), nouveau composant `CartExpressPay.tsx`.
> - **5 bugs réels trouvés et corrigés en testant avec Jake** (détail : `memory/project_bulk_discount_reductions_lot.md`, `memory/feedback_express_payment_pitfalls.md`) : (1) bouton "Ajouter au panier" masquait son texte derrière un spinner jusqu'à 8s en attendant la détection Apple/Google Pay, sans aucun rapport avec sa propre fonction — texte toujours visible immédiatement désormais ; (2) **paiement Apple/Google Pay rejeté à la confirmation quelle que soit la carte** (`on_behalf_of` mismatch Stripe) — l'Elements client ne connaissait jamais le compte Connect du beatmaker que le serveur utilisait pour le PaymentIntent, nouvelle route `/api/stripe/on-behalf-of` pour les synchroniser avant confirmation ; (3) le panier se vidait *avant* la confirmation Stripe dans le nouveau flux panier, démontant le composant Elements en plein milieu (jamais testé jusque-là) — vidé seulement après succès confirmé désormais ; (4) **commande jamais créée** même paiement confirmé : `tentatives_paiement.client_id` recevait l'UID auth d'un beatmaker connecté (aucune ligne correspondante dans `clients`, violation de contrainte FK silencieuse) — nouvelle fonction partagée `resoudreClientId()` (`lib/pricing.ts`), appliquée aux deux routes de checkout ; (5) **cause racine finale, une fois (4) écarté** : le webhook Stripe (`https://beatplatform.vercel.app/api/stripe/webhook`) n'était tout simplement pas abonné à l'événement `payment_intent.succeeded` — le code qui le traite existe et est correct, mais Stripe ne l'envoyait jamais. Trouvé en croisant l'API Stripe (PaymentIntent réussi côté Stripe, aucune `tentative_paiement` correspondante en base, `pending_webhooks:0` sur l'event) avec la config réelle de l'endpoint webhook (`enabled_events` n'incluait pas ce type). Corrigé côté config Stripe (événement ajouté), **validé fonctionnel bout en bout par Jake** (Apple Pay confirmé, commande créée, accès aux fichiers).
>
> Dernière mise à jour : 2026-08-05 — **Étape 5v2 (rang 6) : sélecteur de licence entièrement refondu (nouveau design 2 colonnes) + paiement express Apple Pay/Google Pay/PayPal ajouté, validé fonctionnel sur iPhone Safari réel.** Chantier en 2 lots (décision de séquencement de Jake), tous deux testés bout en bout.
> - **Lot 1 — nouveau design** (`app/[slug]/_components/LicenceSelectorModal.tsx`) : grille 2 colonnes (EXCLUSIVE pleine largeur), badges Populaire/Exclusif, bullets "Inclus avec" en 2 colonnes, footer Total+bouton, overlay non-cliquable (sorties limitées à la croix/Échap). Plusieurs manches de correction pixel-perfect contre le handoff officiel (`handoff_selecteur_licence_v2/licence.css`) — 3 régressions de typographie que j'avais moi-même introduites en sur-corrigeant sur des mesures approximatives (titre/badge/bouton) trouvées et corrigées par mesure `getBoundingClientRect()`/`document.fonts` directe, pas à l'œil. **Bug connexe trouvé et corrigé** : le style/BPM sous le titre du beat ne s'affichait jamais dans le sélecteur — `BeatMin.tag` n'était en fait jamais peuplé par 5 pages différentes de listing (seul `BeatCard.tsx` le dérivait localement pour son propre badge) ; ajouté à chaque `mapBeat()`.
> - **Lot 2 — paiement express** : PaymentIntent Stripe créé au clic (`Elements` mode différé + `ExpressCheckoutElement`), priorité stricte Apple Pay > Google Pay > PayPal (jamais Apple+Google ensemble), prix toujours recalculé serveur (`lib/pricing.ts` extrait de `/api/stripe/checkout`), pipeline de commande/livraison/email entièrement réutilisé (`finaliserCommandePayee()` partagé entre `checkout.session.completed` et le nouveau `payment_intent.succeeded` scopé `metadata.type==='achat_express'`). Fonction pure `selectExpressPaymentMethods()` avec les 8 cas de la table de vérité testés (`scripts/test-express-payments.mjs`). **3 bugs réels trouvés et corrigés en testant avec Jake sur appareil réel** (détail complet : `memory/project_paiement_express.md`) : (1) `IntegrationError` Stripe non interceptée (`layout.overflow:'never'` invalide sans `maxRows:0`) qui bloquait silencieusement l'Element sur TOUS les navigateurs sans jamais déclencher `onReady`/`onLoadError` — trouvé uniquement via la Console Chrome, aucune autre piste de diagnostic exploitable ; (2) minuteur de sécurité 8s inconditionnel qui masquait les boutons même après détection réussie (`useEffect` sans dépendance sur l'état "prêt") ; (3) bulle "Stripe Développeurs" flottante en mode test, masquée via l'option officielle `developerTools.assistant.enabled:false`. Polish UX final : transition douce (couleur/opacité) au lieu du "snap" sur le bouton "Ajouter au panier", état de chargement à 3 valeurs (`loading`/`visible`/`hidden`) avec spinner qui crossfade vers le libellé final.
> - **Reste ouvert, non bloquant** : PayPal peut rester au statut "en attente" côté Dashboard Stripe (email du compte PayPal à confirmer) — à revérifier avant le vrai lancement, sans incidence en mode test. Google Pay pas encore explicitement re-testé par Jake sur un vrai appareil Android/Chrome (confirmé seulement via Chrome desktop + PayPal, et iPhone Safari + Apple Pay/PayPal). Le domaine Stripe (Apple Pay) enregistré sur `beatplatform.vercel.app` devra être ré-enregistré une fois le nom/domaine définitif choisi (voir blocage nom ci-dessous).
>
> Dernière mise à jour : 2026-07-30 (suite x2) — **Bucket "à corriger maintenant" de l'audit 2026-07-29 intégralement clos : T12-T19 tous validés (Claude + Jake), checklist F1-F10 à 100%.** T12 (envoi normal) et T13 (échec total forcé) validés par Jake dans le navigateur — T13 en cassant réellement `RESEND_API_KEY` sur Vercel le temps du test (2 redeploys, clé restaurée à la fin, Jake l'avait gardée de côté avant modification) : campagne test passée en section "Échouées", "Réessayer l'envoi" confirmé fonctionnel après restauration. T14/T15 validés en autonomie par Claude (serveur local + route de diagnostic jamais committée). **4ᵉ bug bonus trouvé en testant (mineur)** : les boutons "Envoyer maintenant"/"Réessayer l'envoi" n'avaient aucun état de chargement, ce qui a fait croire deux fois à Jake qu'un clic n'avait rien fait — corrigé (nouveau composant `EnvoyerCampagneButton.tsx`, pattern `useFormStatus` déjà utilisé ailleurs dans le projet), poussé et vérifié visuellement par Jake. Page "état du projet" simplifiée (artifact) republiée à jour avec la clôture du bucket. **Décision de fin de session** : la suite sera la reprise de la refonte boutique (Étape 5v2, rang 6 — voir "Ordre de priorité actuel") dans une prochaine session ; les 2 items "obligatoires avant le lancement" (litige/chargeback Stripe, historique paiements ratés abonnement plateforme, rang 13) restent en attente, pas urgents tant que la plateforme n'est pas lancée.
>
...
> - **Étape 5v2 (rang 6) — périmètre élargi** : en plus de la personnalisation (hero/couleurs, **déjà testée et validée par Jake** ✅, passe le statut de 🔄 à fait pour cette partie — reste des modifications futures prévues) et du beat cadeau 6.7, Jake a listé d'autres pages boutique jamais explicitement dans le périmètre : `/[slug]/beats` (catalogue complet), la pop-up panier, la fiche produit (page détail beat), la page d'abonnement boutique, la page réservée aux membres, la page panier/validation de commande. L'essai gratuit boutique (bug trouvé par l'audit — le bouton "gratuit" débite immédiatement) devient une option de personnalisation configurable par le beatmaker plutôt qu'un correctif isolé.
> - **Nouveau backlog (hors audit)** : système de rognage (crop) simple du logo à l'upload, demandé par Jake en répondant sur le bug d'upload.
>
> Dernière mise à jour : 2026-07-29 (suite, x2) — **Audit complet de la plateforme demandé par Jake : 4 agents en parallèle (boutique/landing/auth, dashboard beatmaker, module Business, Admin/infra), 21 trouvailles réelles confirmées après recomptage.** 5 critiques (dont un bouton d'abonnement boutique qui débite sans l'essai gratuit promis, 7 pages légales de boutique en fait vides malgré ce que disait cette doc depuis le 2026-07-24, une campagne email qui peut rester bloquée "envoyée" avec 0 destinataire, le statut admin reposant sur un slug renommable sans protection, et un pattern d'email silencieux déjà corrigé une fois en Phase 6 mais reproduit à 2 nouveaux endroits pour les emails de collab), 6 fonctionnels, 9 mineurs, et 1 bonus (la personnalisation boutique par couleurs/hero est en fait déjà codée — corrigé dans la ligne Étape 5v2 ci-dessous). Rapport complet dans deux artifacts (version technique + version vulgarisée) — pas archivés dans ce fichier vu leur taille, mais chaque point sera traité un par un avec Jake (question par question) puis reflété ici au fur et à mesure des décisions.
>
> Dernière mise à jour : 2026-07-29 (suite) — **Roadmap restructuré : refonte UX/UI de toute la plateforme identifiée comme prochaine priorité, en 4 temps (Boutiques → Frontend plateforme → Backend beatmakers → Dashboard beatmaker principal).** Jake a signalé un écart important entre son ressenti ("il reste encore beaucoup plus de choses à faire") et le ROADMAP tel qu'il était formulé. Deux vrais trous de scope confirmés : le **frontend plateforme/landing page** (`app/page.tsx`) et le **dashboard beatmaker principal** (`/dashboard` hors module Business) n'ont jamais été des chantiers ROADMAP à part entière, contrairement à la boutique (5v2) et au module Business (11d) qui ont chacun eu leur refonte. La refonte UX/UI passe désormais devant l'Onboarding et les Tests (rangs 6-9 de l'ordre de priorité, repoussant Onboarding/Tests en 10-11) — inutile de peaufiner un parcours ou tester en profondeur un design appelé à changer. Détail complet dans "Ordre de priorité actuel" et "Trous de scope identifiés par Jake le 2026-07-29". Rien n'est encore cadré en sous-étapes pour cette refonte (comme 5v2/Admin ne l'étaient pas non plus au départ) — à faire dans une prochaine session, en commençant par le rang 7 (Frontend plateforme) puisque le rang 6 (Boutiques) est déjà bien engagé.
>
> Dernière mise à jour : 2026-07-29 — **Chantier terminé et intégralement validé par Jake : confirmation d'adresse email intégrée comme 6ᵉ email My Producer (bug de la bienvenue non-envoyée corrigé), et toute la checklist "Mails My Producer" testée bout en bout (T0-T8, T1a-T1g).** T2-T5 (essai/rappel/paiement/annulation) testés le 2026-07-29 en manipulant Stripe directement (carte de déclin `4000...0341`, `trial_end: 'now'`, annulation immédiate via API) — 2 nuances découvertes en testant, ajoutées aux mémoires `feedback_stripe_test_declines` et checklist : (1) forcer un échec de facture hors-cycle ne suffit pas sur un abonnement encore en essai, il faut `trial_end: 'now'` pour un vrai basculement de statut Stripe ; (2) le bouton d'annulation du portail Stripe programme l'annulation (`cancel_at_period_end`), l'email ne part qu'à l'annulation effective (`customer.subscription.deleted`).
...
> - **En creusant la question, un vrai chantier distinct a émergé** : Jake pensait "Mails transactionnels (admin)" = configurer les emails que **My Producer envoie aux beatmakers eux-mêmes** (bienvenue, confirmation d'abonnement plateforme, rappel fin d'essai, paiement échoué, annulation...) — **zéro n'existe aujourd'hui** (confirmé explicitement hors scope lors du codage de l'Étape 8b le 2026-07-24). Nouveau chantier proposé, "Mails My Producer" — périmètre pas encore validé par Jake, à cadrer avant de coder.
> - **Tests bout en bout (checklist T0-T15), 2 bugs réels trouvés et corrigés en route, sans rapport avec 15d lui-même** : (1) 11 doublons de la catégorie officielle "Jerk" dans `/dashboard/admin/categories`, venus d'un script de clonage de catalogue de test (2026-07-22) qui copiait le statut `certifiee` tel quel vers chaque boutique clonée — nettoyés + `supabase/fix_doublons_categories_certifiees.sql` empêche toute récidive via un index unique ; (2) le compteur "Tendances catalogue" ne filtrait pas les beats supprimés (`supprime_le`), corrigé dans `lib/admin-analytics.ts`. Pour tester le MRR/ARR (T7), fin d'essai forcée sur un abonnement de test via l'API Stripe (`trial_end: 'now'`) plutôt que d'attendre 14 jours.
> - **Retour Jake** : l'onglet Analytics donne une impression de blocs "un peu mélangés" — restructuration visuelle à prévoir dans une prochaine session, pas de détail précis pour l'instant.
> - Checklist de test T0-T15 pour 15d dans la section "Checklist tests Étape 15 lot 2" — **entièrement validée**.
>
...
> - **Bug d'alignement trouvé et corrigé** (retour Jake, hors handoff) : les grilles Nouveautés/Sélection ne s'alignaient pas avec les blocs "Parcourir les styles". Deux causes cumulées : `.shop-catalogue{padding:0}` annulait le gutter desktop (règle morte, retirée) et la grille beats utilisait des colonnes `1fr` élastiques au lieu d'une largeur fixe comme `.shop-style-card` (232px desktop — pas 210px, qui est la valeur mobile).
> - **Bug ouvert à la clôture** : le header mobile (logo+boutons) ne reste pas fixe en haut de l'écran sur le téléphone réel de Jake, malgré un passage `position:sticky`→`position:fixed` validé par test automatisé en local ET sur le site déployé (`beatplatform.vercel.app`). Dev-browser (Chromium headless) ne semble pas reproduire le bug réel — à reprendre en priorité la prochaine session, comparer avec `Downloads/integration-mobile/Boutique Declinaisons Mobile.dc.html`. Détail complet : mémoire `project_boutique_lots_v5_v6_juillet26`.
> - **Test GSAP rejeté** : animation de scroll-reveal (fade+stagger des covers) testée à la demande de Jake, entièrement retirée sur son retour ("trop d'animations") — dépendances `gsap`/`@gsap/react` désinstallées.
>
> Dernière mise à jour : 2026-07-24 — **Étape 8b (suite) : le blocage réel de l'accès dashboard est activé dans `proxy.ts`.** Dernier morceau volontairement différé lors du codage initial d'Étape 8b, pour éviter de reproduire l'incident d'auto-verrouillage admin du même jour. Un beatmaker sans abonnement plateforme `actif`/`en_essai` est désormais redirigé vers `/dashboard/abonnement` dès qu'il touche `/dashboard/**`. Deux exemptions : le compte admin (`SLUG_ADMIN`, déjà exempté par nature) et un nouveau laisser-passer `beatmakers.abonnement_exempte` (migration `supabase/phase8b_3_exemption_gate.sql`), ajouté après un audit qui a montré que **17 des 19 comptes de test existants n'avaient aucun abonnement plateforme** — plutôt que de leur créer un faux abonnement à froid (aurait pollué le futur calcul MRR/ARR de l'Étape 15d), un bouton dédié sur la fiche boutique admin (`/dashboard/admin/boutiques/[id]`) permet d'exempter une boutique de test du gate, activé sur les 15 comptes qui n'ont pas de vrai essai Stripe en cours. **Extension découverte en testant** : la boutique publique (`/{slug}`) était créée/visible dès l'inscription, indépendamment de tout paiement — Jake a tranché que la boutique ne doit exister publiquement que si l'abonnement est payé, et pas juste afficher un message "inactive" (qui confirmerait quand même qu'un slug existe) : **vrai 404** via `notFound()`, comme si la boutique n'existait pas du tout. Même gate (mêmes exemptions) porté sur `app/[slug]/layout.tsx` — le blocage déjà existant pour une boutique suspendue (Étape 15c) reste lui inchangé, avec son message explicite. Checklist de test en fin de section Étape 8b.
>
...
> - **Retour Jake sur la recherche admin** : ralentissait en tapant — segmentée en 3 onglets (Boutiques/Artistes/Commandes & Abonnements, une seule catégorie interrogée à la fois), puis un debounce ajouté (300ms→150ms) a **dégradé** la fluidité perçue au lieu de l'améliorer (mesuré : les requêtes elles-mêmes sont rapides, 50-150ms, le vrai souci était le temps mort artificiel) — retiré complètement, remplacé par un garde-fou de séquence (ignore les réponses obsolètes) qui a aussi corrigé un vrai bug caché ("parfois aucun résultat", race condition sur des réponses arrivées dans le désordre). Détail dans `feedback_recherche_admin_debounce`.
> - **Audit complet demandé par Jake** après avoir buté sur T13/T16 → **découverte que le système d'abonnement plateforme (beatmaker → My Producer) n'a jamais été construit**, malgré la table `abonnements_plateforme` présente depuis le tout premier schéma SQL. Roadmap réordonné (nouvelle **Étape 8b**, priorité 4 — voir mémoire `project_abonnement_plateforme_decouverte`).
> - **Étape 8b codée et testée bout en bout le même jour** : V1 minimale (1 plan mensuel 49,99€/annuel 499,90€, essai 14j + CB obligatoire, accès total ou rien), checkout Stripe direct sans Connect, webhook, page `/dashboard/abonnement`, portail Stripe. Checklist T0-T10 validée. **1 bug trouvé et corrigé** : annuler pendant l'essai ne résilie pas immédiatement côté Stripe (`cancel_at` programmé, `status` reste `trialing`) — sans indicateur, la fiche semblait ignorer l'annulation ; nouvelle colonne `annulation_prevue_le` + affichage ajoutés. **Le blocage réel de l'accès dashboard si non abonné n'est volontairement PAS activé** — prochain chantier à cadrer.
>
...
>
> **⚠️ Incident (2026-07-29) — `admin_role_field.sql` (F2) était resté non exécuté malgré la checklist cochée** : `app/[slug]/layout.tsx` sélectionne `beatmakers.role`, absent tant que la migration n'a pas tourné → requête en échec (`42703`) → `beatmaker` `null` pour **toutes** les boutiques → thème replié sur la couleur bleue par défaut partout, header disparu (`{beatmaker && <BoutiqueHeader/>}`), et gate d'abonnement (Étape 8b) silencieusement contourné le temps de l'incident. Diagnostiqué par requête directe en base (pas en devinant depuis le rendu), migration exécutée par Jake, thèmes et headers de `jakeb-test`/`jakeb-test1-10` revérifiés distincts après coup. Détail : `memory/feedback_verifier_migrations_sql_avant_confirmation.md`. **Leçon retenue : une checklist cochée ne garantit pas qu'une migration a réellement tourné en base — à revérifier par requête directe dès qu'un bug touche tous les comptes/boutiques de façon identique.**
...
>
> **Révision 2026-07-29** : Jake a signalé un écart entre le ROADMAP et ce qu'il a en tête — bien plus de travail restant que ce que la liste précédente laissait penser, centré sur une **refonte UX/UI de toute la plateforme en 4 temps**, avec un ordre précis. Deux trous de scope confirmés au passage : (1) le **frontend plateforme / landing page** (`app/page.tsx`) n'a jamais été une étape ROADMAP à part entière — jamais cadré, jamais retravaillé ; (2) le **dashboard beatmaker principal** (`/dashboard`, hors module Business) n'a pas non plus de chantier UX/UI dédié malgré des mois de fonctionnalités ajoutées sans repasse visuelle. La refonte UX/UI (rangs 6-9 ci-dessous) passe donc devant l'Onboarding et les Tests — inutile de peaufiner un parcours ou de tester en profondeur un design qui va changer juste après.
...
| 5 | **Étape 15 lot 2** — Admin : 15d Analytics plateforme ✅ *(codé et testé bout en bout le 2026-07-27)*, "Mails My Producer" ✅ *(checklist entièrement validée le 2026-07-29, réglages + logs)*, 15e simplifié (voir note) | Repoussé après l'Étape 8b (rang 4) : 15d mesure justement le revenu de l'Étape 8b — le construire avant afficherait des zéros indéfiniment. |
| **6** | **Refonte UX/UI 1/4 — Étape 5v2 : Boutiques beatmakers** 🔄 *(état réel confirmé par Jake le 2026-08-28 — chantier long, la page d'accueil seule a pris plusieurs jours. **Livré** : page d'accueil ✅, personnalisation hero/couleurs ✅, sélecteur de licence + paiement express ✅ (2026-08-05), pop-up panier + réductions par lot + paiement express panier ✅ (2026-08-05), fiche produit mobile+desktop ✅ (2026-08-06/08, retouches légères encore à faire). **Reste, page par page** : pages légales, pages playlists (nouveautés, réservés aux membres, catégories...), FAQ, page "Custom" (beats sur mesure), page "À propos" (présentation du beatmaker), page "Devenir membre", page de validation de commande. Beat cadeau 6.7 toujours ouvert.)* | Ordre fixé par Jake le 2026-07-29 pour la refonte UX/UI complète de la plateforme (rangs 6-9) : c'est la partie la plus avancée, on la termine avant d'attaquer le reste. |
| **7** | **Refonte UX/UI 2/4 — Frontend plateforme (landing page)** 🆕 *(nouvelle étape, jamais cadrée — le "gros trou actuellement" signalé par Jake)* | Le site vitrine de My Producer (`app/page.tsx`, page d'accueil hors `/[slug]` et hors `/dashboard`) n'a jamais eu de vraie passe design/contenu. Périmètre à cadrer avant de coder (sections, message, CTA essai gratuit/inscription). |
| **8** | **Refonte UX/UI 3/4 — Backend beatmakers (Outil Business, `/dashboard/business`)** *(absorbe l'ancienne "11d Phase 8 — accueil")* | Fonctionnel de bout en bout (CRM/Commerce/Analytics/Marketing/Mailing/Catégories) mais jamais retravaillé visuellement depuis la migration crm-proto initiale — periode de polish, pas de nouvelles fonctionnalités. |
| **9** | **Refonte UX/UI 4/4 — Dashboard beatmaker principal (`/dashboard`, hors Business)** 🆕 *(nouvelle étape, jamais cadrée)* | Zone beats/licences/profil/abonnement/splits — fonctionnelle mais jamais retravaillée visuellement malgré des mois d'ajouts. |
| 10 | **Étape 14** — Onboarding | Repositionné après la refonte UX/UI (rangs 6-9, décision 2026-07-29) : le parcours d'inscription doit refléter le design final, pas l'inverse. |
| 11 | **Étape 16** — Tests & corrections bout en bout (absorbe la Phase 4.8) | Après que le plus gros du produit (rangs 1-9) soit construit, design inclus. Phase 4.8 (tests formels Campagnes) déjà décidée comme fusionnée ici plutôt que traitée séparément (note 2026-07-04). |
| 11b | **Étape 16b — Tests de sécurité** 🆕 *(demandée par Jake le 2026-07-29, pas encore cadrée en détail)* | Juste après les tests fonctionnels, juste avant le déploiement — la dernière passe avant qu'un vrai utilisateur touche la plateforme. |
| — | **Étape 11.5** — Import CSV BeatStars (ventes/clients, pas le catalogue de beats) | Parké séparément — bloqué sur l'obtention d'un vrai CSV côté Jake, pas une question d'ordre. **Confirmé le 2026-08-28 : à faire avant la mise en ligne de la plateforme**, pas repoussé indéfiniment. À glisser dès que le CSV est disponible. |
...
| 12 | **11d Phase 4.5** — Vrai domaine d'envoi par boutique | Bloqué tant que le nom/domaine définitif n'est pas choisi (DKIM/SPF/DMARC à configurer une seule fois, pas avant). |
| 13 | **Étape 17** — Déploiement | Bloqué tant que le nom/domaine définitif n'est pas choisi (achat du nom de domaine) + dépend de l'étape 15 (admin) déjà fait. **À ne pas oublier avant le lancement (ajouté 2026-07-29)** : brancher la détection Stripe des litiges/chargebacks sur les commandes (le statut "litige" existe déjà dans l'UI mais rien ne le déclenche), et créer un historique des paiements ratés pour l'abonnement plateforme (existe déjà côté boutique, pas côté plateforme). |
...
>    - Personnalisation par le beatmaker (couleurs, sections, mise en page...) — 🔄 **mise à jour 2026-07-29** : en fait déjà codée pour partie (`/dashboard/business/personnalisation` : titre/sous-titre du hero + 9 couleurs d'accent avec aperçu live, migrations exécutées) — trouvé par l'audit, invisible jusqu'ici car cette ligne disait encore "non défini". Reste à trancher/coder : sections et mise en page (au-delà des couleurs/hero).
...
>
> **Trous de scope identifiés par Jake le 2026-07-29 :**
> 3. **Frontend plateforme / Landing page** — le site vitrine de My Producer lui-même (`app/page.tsx`) n'a jamais été une étape ROADMAP à part, ni retravaillé depuis sa création. Périmètre pas encore défini (sections, ton, CTA) — à cadrer avant de coder, comme l'a été 5v2 en 2026-07-17.
> 4. **Dashboard beatmaker principal (`/dashboard`, hors Business)** — zone beats/licences/profil/abonnement/splits, fonctionnelle mais jamais retravaillée visuellement. Pas de chantier UX/UI dédié jusqu'ici (contrairement au module Business et à la boutique, qui ont eu leur propre refonte).
> 5. **Ordre de priorité demandé par Jake pour la refonte UX/UI** : Boutiques (déjà avancé) → Frontend plateforme → Backend beatmakers (Outil Business) → Dashboard beatmaker principal. Reflété dans "Ordre de priorité actuel" ci-dessus (rangs 6-9), repositionné avant Onboarding/Tests.
>
> **Trou de scope identifié par Jake le 2026-08-04 :**
> 6. **Connexion Google (OAuth)** — jamais évoquée jusqu'ici. Côté artistes (boutiques) : gain de conversion immédiat, coût faible, se greffe sur le mécanisme `lierCompteClient`/fusion de compte déjà existant — candidat pour le rang 6 (refonte boutique). Côté beatmakers : plus structurant (le trigger de création automatique `beatmakers` ne peut pas recevoir de rôle custom via OAuth, il faudra un écran "complète ton profil" après le premier login Google) — recommandé mais proposé pour être traité à l'intérieur de l'Étape 14 (Onboarding, rang 10) plutôt qu'isolément. Détail complet : section "Détail — Connexion Google (OAuth)" juste après l'étape 3 ci-dessous.
...
| 8b | **Abonnement plateforme** *(nouvelle, 2026-07-24)* | Ce que le beatmaker paie à My Producer (`abonnements_plateforme`) — jamais construit malgré la table présente depuis le schéma d'origine. V1 minimale : 1 plan mensuel (49,99€) / annuel (499,90€), essai 14j + CB obligatoire, accès total ou rien. Blocage d'accès réel volontairement différé (lot séparé). Voir mémoire `project_abonnement_plateforme_decouverte`. **"Mails My Producer"** : 6 emails transactionnels plateforme→beatmaker (confirmation email, bienvenue, confirmation essai, rappel fin d'essai J-3, paiement échoué, annulation), template fixe non personnalisable — inexistants jusque-là, trouvé en testant 15d. Bug de la bienvenue jamais envoyée corrigé le 2026-07-28 (inscription passée côté serveur, confirmation email devenue elle-même un email envoyé par nous). Voir mémoire `project_mails_my_producer`. | À estimer | ✅ Codé et testé bout en bout le 2026-07-24 — checklist T0-T10 validée. Reste : blocage d'accès réel (lot séparé). Mails My Producer : checklist T0-T8/T1a-T1g **entièrement validée le 2026-07-29** |
...
| 14 | **Onboarding** | Parcours guidé de configuration à l'inscription. **Note ajoutée le 2026-08-11** : doit intégrer les choix nés du chantier 9 bis actuellement configurés a posteriori dans le dashboard — pages légales/infos légales (`/dashboard/legal`, Phase 1), et plus tard mode de fulfillment + moyens de paiement + identité sur le relevé bancaire (Phase 2). Objectif : que ces décisions soient recueillies au moment de l'inscription plutôt que découvertes séparément par le beatmaker après coup. | 5-8h | ⬜ À faire |
| 15 | **Admin** | Back-office plateforme, périmètre cadré en sous-étapes le 2026-07-24 : **15a** Recherche/Support ✅ codé+testé, **15b** Log Stripe ✅ codé+testé, **15c** Suspendre/Réactiver une boutique (cascade Stripe) ✅ codé+testé (3 bugs trouvés et corrigés pendant les tests), **15d** Analytics plateforme ✅ *(codé et testé bout en bout le 2026-07-27, 2 bugs trouvés et corrigés pendant les tests)*, **15e** Visibilité mails transactionnels 🔄 *(liste détaillée toutes boutiques codée puis retirée le 2026-07-27 — redondante avec les logs déjà propres à chaque beatmaker + trop d'exposition sur les données de leurs clients ; ne reste que le compteur agrégé dans 15d. Le vrai besoin de Jake identifié après coup : "Mails My Producer", à cadrer séparément)*, **15f** Rôles/permissions (reste en V1, gate par slug) — pas prioritaire, **15g** Mises à jour/veille technique ⬜ *(nouvelle zone née le 2026-07-24)*, modération des demandes de certification (catégories, 11d Phase 7) ✅ *(amorcé 2026-07-17, `/dashboard/admin/categories`)*, outil interne d'import BeatStars (script scraping concierge) ⬜. Détail complet + scoring dangerosité : mémoire `project_admin_etape15_scope`. Dépendance dure avant l'étape 17 (déploiement officiel) | 10-15h *(à réviser)* | 🔄 En cours — lot 1 (15a/15b/15c) validé le 2026-07-24, 15d validé le 2026-07-27, 15e simplifié |
| 16 | **Tests & corrections** | Tout tester de bout en bout avant lancement | 8-15h | ⬜ À faire |
| 16b | **Tests de sécurité — session de pentest poussée** *(précisé par Jake le 2026-07-29)* | Pas juste une checklist passive : Jake veut une vraie session de "hacking" volontaire de sa propre plateforme (autorisée, sur son propre système — test d'intrusion défensif classique) menée par Claude, pour trouver et corriger un maximum de failles avant que de vrais utilisateurs arrivent. Périmètre a priori : audit RLS (fuite de données entre boutiques/beatmakers), contrôle d'accès admin (le trou du slug renommable trouvé le 2026-07-29 en est un premier exemple, déjà en cours de correction), résistance du parcours de paiement (falsification de prix/codes promo, rejeu de webhook), protection des uploads/fichiers R2, limitation des tentatives de connexion, injections/XSS sur les champs texte affichés, revue des dépendances. À cadrer précisément et mener comme une session dédiée à part entière, une fois les correctifs connus de l'audit du 2026-07-29 posés (pas de sens à repayer une faille déjà identifiée). | À estimer | ⬜ À faire |
| 17 | **Déploiement** | Mise en ligne sur Vercel + nom de domaine. **À ne pas oublier avant le lancement (2026-07-27)** : nettoyer les abonnements plateforme créés en Stripe test mode (`abonnements_plateforme`) — aucune colonne ne distingue test/live en base, donc ces lignes fausseraient le MRR/ARR de l'Analytics admin (15d) une fois en production si elles restent | 2-4h | ⬜ À faire |
...
## Détail — Connexion Google (OAuth) 🆕 (idée posée par Jake le 2026-08-04)

> Pas encore priorisée dans le tableau "Ordre de priorité actuel" — à positionner une fois le périmètre validé. Candidat naturel pour le rang 6 (refonte boutique, Étape 5v2) côté artistes. Côté beatmakers, probablement à fusionner avec l'Étape 14 (Onboarding, rang 10) plutôt que traité isolément — voir plus bas.

### Côté artistes (boutiques `/[slug]/**`) — gain immédiat, coût faible

**Pourquoi c'est pertinent maintenant** : les `clients` sont déjà partagés entre boutiques (un `auth.users.id` unique relié à plusieurs `leads`, un par boutique visitée), et l'achat n'exige pas de compte (guest checkout, voir `lib/lier-compte-client.ts` et `resoudreOuCreerClient` dans le webhook Stripe). Google réduit la friction exactement là où elle compte : convertir un acheteur invité en compte réel en un clic, et reconnecter un artiste qui a déjà un compte créé sur une autre boutique.

**Ce que ça implique** :
- Config Supabase Dashboard : activer le provider Google (Client ID/Secret Google Cloud Console), redirect URI `https://<projet>.supabase.co/auth/v1/callback`.
- Bouton "Continuer avec Google" sur `app/artiste/connexion/page.tsx` et `app/artiste/inscription/page.tsx` → `supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo } })`.
- `app/auth/callback/route.ts` à étendre : aujourd'hui `lierCompteClient` (création/liaison de la ligne `clients`) est déclenché côté client après un `signUp`/`signInWithPassword` réussi, via un appel explicite à `/api/artiste/lier-compte`. En OAuth il n'y a pas cette étape intermédiaire — il faut appeler `lierCompteClient` directement depuis le callback serveur, en détectant via `next`/`redirect` que le flux vient d'une boutique (slug) et non du dashboard.
- Fusion invité → réel déjà gérée nativement par `fusionner_compte_client` (aucun travail supplémentaire) : un artiste qui a acheté en invité puis se connecte via Google avec le même email récupère automatiquement son historique.
- Plus de cas "email non confirmé" (contrairement au signup email/password actuel) — Google garantit l'email vérifié, ça simplifie un cas limite existant.
- RGPD/confidentialité : mentionner Google comme méthode de connexion dans les pages confidentialité par boutique (email, nom, éventuellement photo de profil récupérés).
- Respecter les guidelines de branding officielles du bouton Google (logo, libellé, couleurs imposées) — pas de bouton "maison".
- Test réel obligatoire sur mobile (Safari iOS notamment) avant de considérer ça fini — `dev-browser` ne suffit pas pour ce genre de flux (mémoire `feedback_mobile_safari_position_testing_limits`).

**Effort estimé** : petit à moyen, comparable aux sous-étapes 3.5/3.6 déjà validées ci-dessus. Le code lui-même est simple ; ce qui prend du temps est le test des cas de fusion de compte / multi-boutique / mobile.

### Côté beatmakers (`/connexion`, `/inscription`, dashboard) — recommandation : oui, mais pas en l'état et pas tout de suite

Question posée par Jake le 2026-08-04. Complications propres au beatmaker, absentes côté artiste :
- La création de la ligne `beatmakers` passe par un **trigger Postgres** (`handle_new_beatmaker`, `supabase/rls_policies.sql` + `supabase/fix_trigger_role.sql`) qui lit `raw_user_meta_data->>'role' = 'beatmaker'`. Ce metadata est injecté par `signUp()` côté mot de passe (`app/api/inscription/beatmaker/route.ts`) — mais `signInWithOAuth()` de Supabase **n'a pas d'équivalent pour y injecter un rôle custom** (seulement `redirectTo`/`scopes`/`queryParams`). Impossible de réutiliser le trigger tel quel pour distinguer un login Google "artiste" d'un login Google "beatmaker".
- Le trigger a aussi besoin de `nom_artiste` pour générer le `slug` — Google ne fournit que nom/prénom/email/photo, pas un nom de marque commerciale.
- Colonnes `beatmakers` NOT NULL non fournies par Google : `cgv_acceptees_at` (acceptation CGV explicite, aujourd'hui une case à cocher du formulaire), `pays`, `devise`. Il faut donc un écran "complète ton profil" après le premier login Google, avant que le compte soit utilisable — un vrai petit flux à concevoir, pas un simple ajout de bouton.
- Risque à surveiller (pas bloquant) : un même `auth.users.id` pourrait porter à la fois une ligne `clients` (achats sur une boutique tierce) et une ligne `beatmakers` (son propre compte vendeur) s'il utilise le même compte Google des deux côtés — déjà possible en théorie avec email/password, mais Google rend ce chevauchement plus probable en pratique. Pas de conflit technique (tables distinctes, même id en clé primaire de chacune), juste à garder en tête pour l'UX ("tu es aussi client de X").

**Recommandation** : oui pour la cohérence produit — pas de raison de laisser les beatmakers sur un formulaire mot de passe si les artistes ont Google, et ça sert aussi la conversion à l'inscription (un des objectifs de l'Étape 14). Mais à traiter comme un vrai sous-chantier avec un écran "complète ton profil" (rôle, nom d'artiste/slug, CGV, pays/devise) — pas un simple ajout de bouton comme côté artiste. Ce sous-chantier recoupe presque entièrement ce que l'Étape 14 (Onboarding, rang 10) doit déjà concevoir de toute façon. Proposition : artistes d'abord (rang 6, gain immédiat, coût faible), beatmakers à l'intérieur de l'Étape 14 plutôt qu'en isolation maintenant — à confirmer avec Jake.
...
>
> Contrairement aux instruments, les tags posés sur `beats.ambiances` ne correspondaient déjà plus aux noms de la table `categories` avant cette migration (données de seed jamais réconciliées). Remap best-effort décidé par Claude (Jake a validé l'approche "comme les instruments" sans trancher les cas un par un) : Energetic/Énergétique→Énergique, Chill/Doux→Calme, Festif→Bouncy, Romantique→Love, Hypnotique→Planant, Nostalgique→Mélancolique. Épique (11 beats) reste orphelin, sans équivalent clair — même traitement que "Cordes" pour les instruments. **Mystérieux** (44 beats) a été repérée par Jake sur une capture ("je vois une catégorie qui est restée mystérieux, retire la de toutes les boutiques", 2026-07-31) : au lieu de la laisser orpheline, le tag est supprimé des beats (`array_remove`), pas juste masqué à l'affichage.
>
...
#### Checklist tests Étape 8b (suite) — Gate d'accès dashboard ✅ Validée le 2026-07-24

> **Contexte :** Dernier morceau volontairement différé lors du codage initial (voir T10 ci-dessus) — activé le 2026-07-24 dans `proxy.ts`, une fois le paiement lui-même testé. Un beatmaker sans abonnement `actif`/`en_essai` est redirigé vers `/dashboard/abonnement`. Deux exemptions : `SLUG_ADMIN` (compte admin) et le nouveau laisser-passer `beatmakers.abonnement_exempte` (migration `supabase/phase8b_3_exemption_gate.sql`).
>
...
- [x] **T19** — Un compte `suspendu` reste redirigé vers `/dashboard/suspendu` en priorité, même s'il a par ailleurs un abonnement actif ou une exemption (le check suspension passe avant le check gate abonnement)
...
#### Checklist tests Étape 15 lot 2 — Admin 15d Analytics plateforme ✅ Validée le 2026-07-27

> **Contexte :** Suite du lot 1 (15a/15b/15c, validé le 2026-07-24), repoussée après l'Étape 8b (déjà validée) pour que 15d mesure un vrai revenu plutôt que des zéros. Aucune migration SQL nécessaire — la page lit uniquement des tables déjà existantes (`abonnements_plateforme`, `commandes`, `beatmakers`, `email_logs`, `stripe_events`, `beats`).
>
> ⚠️ **15e (vue détaillée des emails toutes boutiques) retirée en cours de session** après discussion avec Jake — jugée redondante avec les logs déjà propres à chaque beatmaker (`/dashboard/business/marketing/logs`) et trop exposée sur les données de leurs clients (principe de minimisation RGPD). Ne reste que le compteur agrégé du bloc "Santé technique" ci-dessous. Détail : mémoire `project_admin_etape15_scope`.
>
> **2 bugs réels trouvés et corrigés pendant les tests** (aucun des deux dans le périmètre 15d lui-même) : (1) 11 doublons de la catégorie officielle "Jerk" dans `/dashboard/admin/categories`, causés par un ancien script de clonage de catalogue (test1-10, 2026-07-22) qui copiait le statut `certifiee` tel quel — nettoyés + `supabase/fix_doublons_categories_certifiees.sql` ajoute un index unique qui empêche structurellement toute récidive ; (2) le compteur "Tendances catalogue" ne filtrait pas les beats supprimés (`supprime_le`), incohérent avec le reste de l'app — corrigé dans `lib/admin-analytics.ts`. Détail complet : mémoire `project_admin_etape15_scope`.
>
> **Retour Jake en fin de session** : l'onglet Analytics plateforme donne une impression de blocs "un peu mélangés" — restructuration visuelle à prévoir dans une prochaine session (pas de détail précis donné pour l'instant).

**Accueil admin :**
- [x] **T0** — `/dashboard/admin` affiche la nouvelle carte "Analytics plateforme", lien fonctionnel

**Analytics plateforme (`/dashboard/admin/analytics`) :**
- [x] **T7** — Bloc "Ton revenu réel" : MRR/ARR cohérents avec les abonnements plateforme réellement actifs *(validé après avoir forcé la fin d'essai d'un abonnement de test via l'API Stripe — `trial_end: 'now'` — pour obtenir un vrai statut `actif` à mesurer ; MRR/ARR à 0 avant ça confirmé normal, un essai ne compte pas comme du revenu récurrent)*
- [x] **T8** — Compteurs actifs/en essai/annulés/impayés cohérents avec le nombre réel de lignes `abonnements_plateforme` par statut
- [x] **T9** — Bloc "Volume d'affaires boutiques" : CA net total cohérent avec la somme des CA net de chaque boutique
- [x] **T10** — CA net "30 derniers jours" plausible (inférieur ou égal au total tout historique)
- [x] **T11** — Bloc "Croissance" : nombre de boutiques actives cohérent (exclut les boutiques suspendues), nouveaux beatmakers 7j/30j plausibles
- [x] **T12** — Bloc "Classement des boutiques" : ordre décroissant par CA net correct, lien vers la fiche boutique fonctionne
- [x] **T13** — Bloc "Santé technique" : le compteur d'échecs email (7j) correspond au nombre réel de lignes `echoue` dans `email_logs` sur la période *(vérifié : 22 échecs email au total, mais 0 sur les 7 derniers jours — donc 0 affiché est correct, pas un bug)* ; le compteur webhook + son lien vers `/dashboard/admin/stripe-events?filtre=echoue` fonctionnent
- [x] **T14** — Bloc "Tendances catalogue" : total de beats (375) expliqué — catalogues clonés sur les 10 boutiques de test (33 beats × 11 comptes `jakeb-test`/`jakeb-test1-10`), pas un vrai catalogue plateforme
...
**Annulation :**
- [x] **T5** — Annuler un abonnement plateforme de test → email de confirmation reçu. ⚠️ Nuance trouvée en testant : le bouton d'annulation du **portail Stripe** programme `cancel_at_period_end` (annulation différée) — l'email ne part que sur l'événement `customer.subscription.deleted` (annulation **effective**), pas au moment où l'annulation est programmée. Annulation immédiate forcée via l'API (`stripe.subscriptions.cancel`) pour déclencher le vrai événement et valider l'email.
...
#### Checklist tests correctifs audit 2026-07-29 (F1-F10) ✅ T1-T19 tous validés (Claude + Jake) — bucket clos le 2026-07-30
...
>
> **3 bugs bonus trouvés et corrigés en testant T11/T16 avec Jake, sans rapport direct avec F1-F10** : (1) le nom du collaborateur restait vide sur `/dashboard/business/beats/[id]/modifier` — RLS (`beatmakers_select_own`) bloquait la lecture du `nom_artiste` d'un autre beatmaker via le client authentifié, corrigé en passant cette requête au client admin (le beat est déjà vérifié comme appartenant à l'utilisateur juste avant). (2) Chaque sauvegarde d'un beat supprimait puis recréait toutes ses lignes `beat_splits`, même sans toucher aux collaborateurs — comme `split_payments.beat_split_id` référence `beat_splits` en `on delete set null`, ça orphelinait silencieusement l'historique de paiements déjà reçus par le collaborateur à *chaque* édition du beat (`/dashboard/business/collabs` affichait "Pas encore vendu" malgré des paiements réels). Corrigé par une réconciliation par id (suppression seulement des collaborateurs réellement retirés, mise à jour en place du reste). 17 paiements déjà orphelins (compte de test "Nico beats") réattachés manuellement après coup ; 27 autres orphelins ailleurs sur la plateforme (jakeb-test collaborateur sur d'autres boutiques de test) laissés tels quels sur décision de Jake — corriger la cause suffit, pas de rattrapage risqué sans certitude d'attribution. (3) **Le plus grave, trouvé en cherchant pourquoi rien ne se passait sur T11** : les 3 endroits qui gèrent les fonds en attente des collaborateurs (déblocage manuel `/api/stripe/splits/debloquer`, déblocage auto à la connexion Stripe dans le webhook, cron quotidien `splits-expiration` J+30/J+50/J+60) sélectionnaient tous `commandes(beats(titre))` — une relation supprimée depuis le passage au panier multi-articles (Phase 2c, 2026-07-09). Comme aucune de ces requêtes ne vérifiait l'erreur retournée, chaque échec était traité silencieusement comme "rien à traiter" : **plus aucun transfert Stripe collab (manuel ou auto), aucun reversal J+60, aucun rappel J+30/J+50 ne partait réellement depuis 3 semaines**, sans la moindre trace d'erreur nulle part. Corrigé en récupérant le titre via `beat_split_id → beat_splits → beats` + vérification d'erreur avec log sur les 3 requêtes. Vérifié en conditions réelles : après correction, les 3 paiements en attente de "COLLABS JAKE" (test collab de `jakeb-test`) se sont transférés avec succès pour de vrai.

> **4ᵉ bug bonus (mineur), trouvé en testant T13 le 2026-07-30** : les boutons "Envoyer maintenant" / "Réessayer l'envoi" (`CampagnesClient.tsx`) n'ont aucun état de chargement — le bouton reste cliquable et rien ne bouge visuellement pendant la requête. Ça a fait croire à Jake une première fois qu'un clic n'avait rien fait (l'a poussé à recliquer sur une campagne déjà envoyée, bloquée par la protection anti-double-envoi existante — donc pas un vrai risque, juste confusion) et une seconde fois pendant un "Réessayer" qui échouait de nouveau (résultat visuellement identique à l'état précédent, donc imperceptible sans indicateur). Pas encore corrigé — proposé à Jake (état "Envoi en cours…" + désactivation pendant la requête), en attente de sa confirmation.
...
- [x] **T11** — ✅ Testé par Jake le 2026-07-30, indirectement : en creusant pourquoi rien ne s'affichait sur `/dashboard/paiements?connected=true`, **3 bugs distincts trouvés et corrigés en route** (aucun n'est F4 lui-même) — voir note détaillée ci-dessus. Une fois corrigés, les 3 paiements en attente de "COLLABS JAKE" (18,75€/37,50€/37,50€) se sont **réellement transférés avec succès** (confirmé sur `/dashboard/business/collabs`, 118,75€ reçus au total) — le vrai échec forcé (solde insuffisant) n'a pas été nécessaire/déclenché, donc le message d'erreur de F4 lui-même reste non exercé en conditions réelles, mais son code est cohérent avec les autres correctifs déjà validés (Jake a accepté ce niveau de confiance plutôt que de forcer artificiellement un échec)

> **Nouvelle fonctionnalité ajoutée dans la foulée (demandée par Jake)** : `/dashboard/paiements` n'avait jusqu'ici aucune visibilité sur les fonds collab en attente — le seul déclencheur du déblocage était de recliquer sur "Compléter la configuration Stripe" (redirection Stripe Connect avec `?connected=true`), sans bouton dédié ni rapport. Ajouté : section "Fonds en attente" (total + nombre de paiements), bouton "Débloquer mes fonds en attente" indépendant du flux Connect, et un vrai rapport après clic (paiements débloqués en vert avec beat+montant, échecs en orange avec l'erreur). **Testé et validé par Jake le 2026-07-30** sur le compte "Nico beats" : 2 paiements (25€) débloqués avec succès, rapport affiché correctement ("TEST SPLIT — 12,50€" x2).

**F1 — Campagne "échouée" à 0 destinataire :**
- [x] **T12** — ✅ Validé par Jake le 2026-07-30 : campagne "test 3007" (7 destinataires) envoyée normalement, apparaît dans la section "Historique" de `/dashboard/business/marketing/campagnes` (qui ne liste que les campagnes `statut = 'envoyee'` — le statut est encodé par la section elle-même, pas par un badge/colonne dédié ; une campagne échouée apparaîtrait dans la section rouge "Échouées" avec le bouton "Réessayer l'envoi")
- [x] **T13** — ✅ Validé par Jake le 2026-07-30, en cassant réellement `RESEND_API_KEY` sur Vercel (deux redeploys, restaurée à la fin) : campagne "fgfgj" (7 destinataires) passée en section rouge **"Échouées"** avec le bon message + bandeau d'erreur détaillé ("vérifie la config Resend — variables d'environnement, domaine d'envoi"). "Réessayer l'envoi" cliqué une 1ère fois clé toujours cassée (aucun changement visible — attendu, l'état échec→échec est identique visuellement, cause du signalement du bug bonus ci-dessous), puis une 2ème fois après redeploy avec la vraie clé restaurée → campagne repassée en "Envoyée" dans Historique, confirmant le bon fonctionnement du bouton. Suppression non re-testée explicitement (capacité générique déjà validée par ailleurs, `neq('statut','envoyee')`)

**F3 — Emails de collab migrés (le plus lourd, dernier) :**
- [x] **T14** — ✅ Testé par Claude le 2026-07-30 (serveur `next dev` local + route de diagnostic jamais committée, jamais ouvert dans un vrai navigateur) : `genererApercuTransactionnelPlateforme` génère un HTML valide pour les 4 types collab, avec ou sans titre/intro personnalisés — confirme que le mécanisme d'aperçu (partagé avec les 6 types existants déjà validés) fonctionne aussi pour ceux-ci. **Reste un point non couvert par ce test** : l'ouverture réelle des accordéons et le rendu de l'iframe dans le navigateur (comportement UI générique, jamais spécifique à ces 4 types, mais pas vérifié à l'œil)
- [x] **T15** — ✅ Testé par Claude le 2026-07-30, même méthode : titre/intro custom écrits dans `templates_plateforme` pour `collab_fonds_attente`, puis `envoyerFondsEnAttente()` appelée réellement — email envoyé (reçu sur `nicojacob83+collab@gmail.com`) avec le sujet et l'intro personnalisés au lieu des valeurs par défaut, confirmé via la ligne `email_logs` créée. Donnée de test nettoyée après coup (ligne `templates_plateforme` et `email_logs` supprimées, état identique à avant le test)
...
| 2026-08-04 | Session de planification (pas de code) — Connexion Google (OAuth) cadrée | Jake a proposé d'ajouter la connexion Google pour les artistes sur les boutiques, jamais évoquée jusqu'ici. Exploration de l'architecture auth existante (`lib/lier-compte-client.ts`, trigger `handle_new_beatmaker`, `app/auth/callback/route.ts`, guest checkout) pour cadrer l'implication réelle avant de coder. **Côté artistes** : se greffe proprement sur le mécanisme `lierCompteClient`/fusion de compte invité→réel déjà existant, coût faible — ajouté comme candidat au rang 6 (refonte boutique, Étape 5v2). **Côté beatmakers** : plus structurant — le trigger Postgres qui crée la ligne `beatmakers` dépend d'un metadata `role` injecté par `signUp()` classique, que `signInWithOAuth()` ne peut pas fournir de la même façon, et Google ne fournit ni `nom_artiste`/slug ni acceptation CGV ni pays/devise (colonnes NOT NULL) — nécessite un écran "complète ton profil" post-login. Recommandation : oui sur le principe, mais à traiter à l'intérieur de l'Étape 14 (Onboarding, rang 10) plutôt qu'en isolation maintenant. Rien codé — cadrage ajouté dans une nouvelle section "Détail — Connexion Google (OAuth)" (après l'étape 3) + point 6 des trous de scope, en attente de priorisation par Jake. |
| 2026-08-05 (suite) | Réductions par lot ✅ + refonte popup panier ✅ + paiement express panier multi-articles ✅ — 5 bugs trouvés et corrigés | **Réductions par lot** ("achète X, obtiens Y offert", parti du handoff `handoff_panier_v1`) : table `reductions_lot` (une règle = une licence, une seule active à la fois par licence — contrainte unique en base), calcul entièrement dans `lib/pricing.ts` (article offert = le moins cher éligible, dernier ajouté en cas d'égalité — décision de Jake après un premier essai où le décompte du bandeau était mal formulé), cumul avec code promo configurable par code (`codes_promo.cumulable_reduction_lot`), dashboard `/dashboard/business/reductions-lot`. **Activée par défaut sur toutes les boutiques** (backfill + trigger `creer_licences_defaut()` étendu), noms de licences par défaut simplifiés (MP3/WAV/STEMS/ILLIMITÉ/EXCLUSIVE). **Refonte popup panier** (`CartDrawer.tsx` mobile+PC) : popup blanche volontaire, pixel-perfect contre la maquette live via `dev-browser`, bandeau promo + "GRATUIT" affichés en temps réel (`app/[slug]/_lib/reductions-lot.ts`, calcul miroir côté client, affichage uniquement). **Paiement express étendu au panier** (`CartExpressPay.tsx`, `/api/stripe/express-checkout` généralisé à `items[]`) — décision de Jake de dupliquer exactement la logique déjà validée du sélecteur de licence. **5 bugs réels trouvés et corrigés en testant avec Jake** : (1) bouton "Ajouter au panier" masqué par un spinner jusqu'à 8s en attendant la détection Apple/Google Pay sans rapport avec sa fonction ; (2) paiement Apple/Google Pay rejeté à la confirmation quelle que soit la carte (`on_behalf_of` mismatch — l'Elements client ne connaissait jamais le compte Connect du beatmaker utilisé côté serveur, nouvelle route `/api/stripe/on-behalf-of`) ; (3) panier vidé avant la confirmation Stripe dans le nouveau flux panier, démontant le composant Elements en plein milieu (jamais testé jusque-là, trouvé en anticipant plutôt qu'en attendant le rapport de Jake) ; (4) commande jamais créée malgré paiement confirmé — `tentatives_paiement.client_id` recevait l'UID d'un beatmaker connecté (aucune ligne correspondante dans `clients`, violation de FK silencieuse), nouvelle fonction partagée `resoudreClientId()` appliquée aux deux routes de checkout ; (5) **cause racine finale** une fois (4) écarté (Jake confirmé non connecté) : le webhook Stripe n'était pas abonné à l'événement `payment_intent.succeeded` — trouvé en croisant l'API Stripe (PaymentIntent réussi, aucune tentative correspondante en base, `pending_webhooks:0`) avec la config réelle de l'endpoint (`enabled_events` n'incluait pas ce type). Corrigé côté config Stripe, **validé fonctionnel bout en bout par Jake** (Apple Pay confirmé, commande créée, fichiers accessibles). Détail complet : `memory/project_bulk_discount_reductions_lot.md`, `memory/feedback_express_payment_pitfalls.md`. |
| 2026-08-04/05 | Étape 5v2 — Sélecteur de licence : nouveau design + paiement express Apple Pay/Google Pay/PayPal ✅ | **Lot 1 (design)** : handoff `handoff_selecteur_licence_v2/licence.css` intégré dans `LicenceSelectorModal.tsx` — grille 2 colonnes (EXCLUSIVE pleine largeur), badges Populaire/Exclusif, bullets 2 colonnes, footer Total+bouton, overlay non-cliquable (croix/Échap uniquement). Plusieurs manches de correction pixel-perfect avec Jake (captures annotées + prompts ChatGPT dédiés) contre la maquette — 3 régressions typographiques que j'avais moi-même introduites en sur-corrigeant sur des mesures approximatives, retrouvées et corrigées par mesure directe (`getBoundingClientRect`/`document.fonts`), pas à l'œil. **Bug connexe** : le style/BPM sous le titre du beat ne s'affichait jamais dans aucune des 5 pages de listing (`BeatMin.tag` jamais peuplé, seul `BeatCard.tsx` le dérivait localement) — corrigé partout. **Lot 2 (paiement express)**, plan validé par Jake avant codage : `PaymentIntent` créé au clic via `Elements` en mode différé + `ExpressCheckoutElement` (pas de Checkout Session, montant pas modifiable après coup) ; priorité stricte Apple Pay > Google Pay > PayPal, jamais les deux wallets ensemble, via `selectExpressPaymentMethods()` (fonction pure, 8/8 cas testés dans `scripts/test-express-payments.mjs`) ; prix toujours recalculé serveur (`lib/pricing.ts`, extrait de `/api/stripe/checkout`) ; pipeline commande/livraison/email réutilisé sans duplication (`finaliserCommandePayee()` partagé entre `checkout.session.completed` classique et le nouveau `payment_intent.succeeded` scopé `metadata.type==='achat_express'`) ; migration `supabase/express_checkout.sql` exécutée et vérifiée en base par requête directe (pas seulement confirmation verbale). **3 bugs réels trouvés et corrigés en testant avec Jake sur iPhone Safari + Chrome desktop réels** (détail : `memory/project_paiement_express.md`, `memory/feedback_stripe_layout_overflow_maxrows.md`) : (1) le plus long à diagnostiquer — `IntegrationError` Stripe non interceptée (`layout.overflow:'never'` invalide sans `maxRows:0`) qui empêchait silencieusement l'Element de s'initialiser sur TOUS les navigateurs/appareils, sans jamais déclencher `onReady` ni `onLoadError` ; config Dashboard Stripe vérifiée 100% correcte en parallèle (domaine Apple Pay, PayPal, Google Pay) avant de trouver la vraie cause via la Console Chrome (F12) sur suggestion — trouvée seulement après avoir demandé à Jake de coller l'erreur brute ; (2) minuteur de secours 8s qui masquait les boutons même après détection réussie (`useEffect` sans dépendance sur l'état "prêt", corrigé) ; (3) bulle "Stripe Développeurs" flottante, propre au mode test (confirmée non-bug via la doc Stripe officielle), masquée via `developerTools.assistant.enabled:false`. Polish UX final demandé par Jake : transition douce (au lieu du "snap") sur le bouton "Ajouter au panier" au changement de style, état de chargement à 3 valeurs (`loading`/`visible`/`hidden`) avec spinner qui crossfade en douceur vers le libellé final. **Validé fonctionnel bout en bout par Jake sur iPhone Safari réel** (Apple Pay + PayPal). **Reste ouvert, non bloquant** : statut PayPal "en attente" côté Dashboard à revérifier avant lancement (email compte PayPal non confirmé, sans incidence en mode test) ; Google Pay pas encore re-testé par Jake sur un vrai appareil Android/Chrome. Détail complet des décisions et de l'architecture : `memory/project_paiement_express.md`. |
| 2026-08-08/11 | Revue juridique/fiscale — risque article 9 bis (fournisseur réputé), plan de refonte en 13 phases | Session longue déclenchée par une question de fond de Jake sur le risque que My Producer soit qualifiée de fournisseur réputé (art. 9 bis, règlement TVA UE 282/2011) — scénario qui forcerait 20% de TVA sur toutes les ventes de tous les beatmakers, déjà vécu sur BeatStars. **Audit initial** : destination charge Stripe (pas direct), CGV/mentions légales encore vides sur toutes les boutiques, bouton "Rembourser" qui ne fait qu'un flag DB (jamais Stripe), licences figées dans le code, aucune facture jamais générée — plus un bug trouvé au passage (reversement collab silencieusement bloqué 3 semaines, 09/07-29/07). **Analyse multi-Direct-Charge** (splits) : coûts réels identifiés (auth bancaire potentiellement dupliquée, pas de garantie d'atomicité, frais croissants avec le nombre de collaborateurs). **Comparatif Stripe vs PayPal Multiparty** pour les ventes collaboratives : PayPal mieux disant sur la détention des fonds mais nécessite un statut partenaire non garanti pour la France/UE. **Grill Me** (19 objections critique/important/à cadrer, tranchées une par une, plusieurs décisions initiales corrigées). **Plan en 13 phases** audité en retour par Jake (9 corrections intégrées) puis découpé en tâches atomiques. **Architecture cible** : "le beatmaker décide, My Producer exécute techniquement" — Direct Charge pour les ventes simples, mandat de fulfillment, snapshot transactionnel par commande, mandat commercial collaborateurs, licences/CGV éditables par beatmaker, `merchant_decisions_log`. Le choix du processeur pour les ventes collaboratives (Phase 13) est volontairement gelé en dernier, en attendant la réponse du dossier partenaire PayPal — ne bloque aucune des 12 autres phases. **Phase 1 (pages légales réelles) démarrée** : `supabase/pages_legales.sql` (table `boutique_pages_legales`, une ligne par beatmaker/type de page, versionnée). Documents produits (hors dépôt) : Audit 9 bis, Multi-Direct-Charge, Stripe vs PayPal, Grill Me, Plan d'implémentation, Découpage en tâches. |
