...
| 5.7 | Combinaisons : règles de résolution (état net, slots, templates combinés fréquents). **À réintégrer ici** (retirées le 2026-07-14 car prématurées pendant la phase d'isolation) : la règle de suppression de Bienvenue perso (rien d'autre le même jour — voir 5.6e) et le garde-fou anti-achat de Follow-up free download (voir 5.6g) — la fonction `doitEtreIgnore()` dans `lib/automatisations.ts` a été entièrement supprimée, à recréer ici | ⬜ |
