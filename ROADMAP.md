...
>
> **⚠️ Point de vigilance scaling (identifié le 2026-07-04, pas urgent) :** `/api/cron/automatisations` traite les événements en attente **séquentiellement, un par un** (boucle `for` avec `await`), dans une seule exécution bornée par le temps max Vercel (5 min par défaut sur Hobby, jusqu'à 30 min sur Pro en configuration étendue). À ~1s/événement (dominé par l'appel réseau Resend), ça tient large jusqu'à ~300 emails par passage sur Hobby. Le vrai risque n'est pas "beaucoup de boutiques" en soi mais **beaucoup de boutiques dont l'heure cible par défaut se recoupe** (ex. 10h15) — le jour où plusieurs centaines de boutiques actives partagent la même fenêtre d'envoi, un seul passage du cron pourrait ne pas avoir le temps de tous les traiter. Fix pas urgent mais à prévoir avant la vraie mise à l'échelle : traiter les événements par lots en parallèle (`Promise.all` avec limite de concurrence) plutôt qu'un par un, et/ou plafonner le nombre traité par passage pour étaler la charge.
