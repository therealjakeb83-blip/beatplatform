...
> - **Chantier séparé cadré le 2026-08-28, planifié APRÈS ce chantier juridique/fiscal (pas une phase du plan 9 bis)** : réglage de fuseau horaire par beatmaker — découvert via un bug d'affichage (dates -2h sur les pages calculées côté serveur), estimé 4-7h. Deux volets : affichage (11 pages Server Component identifiées) et, plus important, le découpage des périodes dans les Analytics (calcul de "ce mois-ci" etc. fait en UTC côté serveur au lieu du fuseau du beatmaker — un vrai problème de regroupement des données, pas juste d'affichage). Détail complet : `memory/project_fuseau_horaire_par_beatmaker.md`.
> - **Statut actuel (2026-08-28) : Phases 1 à 5 entièrement terminées et validées (T5/T6 d'immuabilité de la Phase 4 exécutés — voir entrée dédiée plus bas ; Phase 5 close en 5 lots — statut/affichage/reprise/alerte). Une partie de la Phase 9 (TVA toujours absorbée) traitée par anticipation le même jour — voir entrée dédiée.** Phase 1 : `supabase/pages_legales.sql` (table `boutique_pages_legales` — une ligne par `beatmaker_id`/`type_page` parmi cgv/mentions_legales/confidentialite/contact/plan_de_site, versionnée, RLS par beatmaker ; sans ligne, le template par défaut de `lib/pages-legales.ts` s'affiche). Distincte de `beatmakers.cgv_acceptees_at`, qui concerne l'acceptation des CGV de My Producer par le beatmaker (relation SaaS), pas les CGV que le beatmaker publie vers ses propres clients. Phase 2 : bascule complète vers Direct Charge, toutes les boutiques — voir entrée détaillée du 2026-08-27 plus bas. Phase 3 : remboursement Stripe réel pour les ventes solo Direct Charge, ventes collab volontairement bloquées — voir entrée détaillée du 2026-08-28 plus bas. Reste les phases 4 à 12 à cadrer/coder, **Phase 13 volontairement gelée** en attendant la réponse du dossier partenaire PayPal.
