...
> - **Statut actuel (2026-08-27) : Phases 1 et 2 terminées et validées. Phase 3 (remboursement Stripe réel) à démarrer.** Phase 1 : `supabase/pages_legales.sql` (table `boutique_pages_legales` — une ligne par `beatmaker_id`/`type_page` parmi cgv/mentions_legales/confidentialite/contact/plan_de_site, versionnée, RLS par beatmaker ; sans ligne, le template par défaut de `lib/pages-legales.ts` s'affiche). Distincte de `beatmakers.cgv_acceptees_at`, qui concerne l'acceptation des CGV de My Producer par le beatmaker (relation SaaS), pas les CGV que le beatmaker publie vers ses propres clients. Phase 2 : bascule complète vers Direct Charge, toutes les boutiques — voir entrée détaillée du 2026-08-27 plus bas. Reste les phases 3 à 12 à cadrer/coder, **Phase 13 volontairement gelée** en attendant la réponse du dossier partenaire PayPal.
