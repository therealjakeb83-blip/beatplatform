...
> Dernière mise à jour : 2026-07-14 — **Phase 5.7 (combinaisons entre workflows) codée, en attente de test par Jake.** Session de revue exhaustive des 21 scénarios de combinaison possibles entre les 7 workflows (docs/automatisations/combinaisons-5.7.md), tranchés un par un avec Jake, puis codés en autonomie. Conclusion de la revue : **aucun cas rare** — tout se résout de façon déterministe, **Phase 5.8 (IA) n'est donc pas construite** (pas de cas d'usage réel). `lib/automatisations.ts` entièrement restructuré autour d'un système de résolution "par jour et par client" à 2 passes (l'argent d'abord — Abonnement puis Achat —, puis seulement si rien n'en sort : Relance > Free download > Bienvenue perso), remplaçant le traitement événement-par-événement d'avant 5.7. Détail complet (algorithme, garde-fous, correctifs) dans le doc et dans `memory/project_phase5_combinaisons_implementation.md`. **Non testé en conditions réelles à ce stade** — Jake doit valider demain.
>
...
- **Une seule vraie combo codée en dur** au final : Achat + Bienvenue abo (`combo_achat_abonnement_bienvenue`). Toutes les autres combinaisons envisagées à l'origine (Achat+Abonnement en attente, notamment) ont été révisées en cours de revue vers une simple priorité/domination plutôt qu'un template fusionné.
...
| 5.9 | Tests bout en bout de 5.7 (scénarios réels : achat+abo bienvenue le même jour, abo en attente redevenu actif avant l'envoi, plusieurs achats/téléchargements le même jour, bienvenue perso après un ancien guest checkout...) | ⬜ *(à faire par Jake)* |
