...
| 16b | **Tests de sécurité** *(nouvelle, demandée par Jake le 2026-07-29)* | Passe de sécurité dédiée avant le lancement — périmètre pas encore cadré en détail, mais couvre a priori : audit RLS (fuite de données entre boutiques/beatmakers), contrôle d'accès admin (le trou du slug renommable trouvé le 2026-07-29 en est un premier exemple), résistance du parcours de paiement (falsification de prix/codes promo, rejeu de webhook), protection des uploads/fichiers R2, limitation des tentatives de connexion, et une revue des dépendances (recoupe la zone 15g jamais construite). À cadrer précisément dans une prochaine session, comme l'ont été 5v2/Admin/la refonte UX/UI. | À estimer | ⬜ À faire |
