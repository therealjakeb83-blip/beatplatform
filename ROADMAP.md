...
> Dernière mise à jour : 2026-07-16 — **Phase 5.7 (combinaisons entre workflows) codée, tests en cours.** Revue exhaustive des 21 scénarios de combinaison possibles entre les 7 workflows (docs/automatisations/combinaisons-5.7.md), tranchés un par un avec Jake, puis codés en autonomie. Conclusion de la revue : **aucun cas rare** — tout se résout de façon déterministe, **Phase 5.8 (IA) n'est donc pas construite** (pas de cas d'usage réel). `lib/automatisations.ts` entièrement restructuré autour d'un système de résolution "par jour et par client" à 2 passes. Détail complet dans le doc et dans `memory/project_phase5_combinaisons_implementation.md`. **Test 1 (combo achat+abonnement) validé par Jake** — reste les scénarios 2-17 de la checklist 5.9.
>
