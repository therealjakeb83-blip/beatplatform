...
>
> **Bug corrigé (2026-07-06) — course entre le webhook et la redirection navigateur :** en testant en conditions réelles, découvert que `invoice.payment_succeeded` (webhook, quasi instantané) pouvait arriver avant que `/api/stripe/abonnement/succes` (redirection après paiement, dépendante du navigateur du client — pas garantie) ait fini de créer la ligne `abonnements_boutique`. Résultat : la commande de création et l'incrément des compteurs échouaient silencieusement (confirmé en renvoyant le même événement plus tard : ça fonctionnait alors). Corrigé en déplaçant la création de l'abonnement dans le webhook (`checkout.session.completed`, fiable, idempotent) — `/api/stripe/abonnement/succes` ne fait plus que poser le cookie de session membre et rediriger, conformément à la recommandation Stripe (`success_url` = UX uniquement, jamais de logique critique). Même pattern que le flux d'achat de beat classique, qui faisait déjà ça correctement.
