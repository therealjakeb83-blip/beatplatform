...
| 2c.4 | Webhook `traiterPaiement` : 1 insert `commandes` (header) + N inserts `commande_lignes`, splits/contrat PDF par ligne (`distribuerSplitsArticle`), check "1er achat" évalué **une fois par session** (pas par ligne), incrément `codes_promo.utilisations` une fois par commande, token `{{le_beat}}` recompté via `commande_lignes` | ✅ |
...
| 5.6d | Remerciement achat | Scindé en 2 automations distinctes (décision Jake, 2026-07-09) : **1er achat** codé (ce tableau), **paliers client récurrent** (2e/3e/4e et +, basé sur données réelles : 1.5 commande/client en moyenne, 3.1 chez les récurrents) reporté à une session dédiée — voir note ci-dessous | 🔄 *(1er achat codé, à tester)* |
...
| 5.2 | Les 8 workflows en isolation (contenu + déclencheur) — **construits un par un** : 3/8 validés (Bienvenue abonnement, Abonnement en attente, Churn message perso) | 🔄 |
