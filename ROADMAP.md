...
> Dernière mise à jour : 2026-07-17 — **Phase 6 (Mailing transactionnels), sous-étapes 6.1 à 6.6 validées bout en bout.** 4 emails temps réel fonctionnels et personnalisables (confirmation commande, confirmation abonnement, demande d'annulation avec date de fin d'accès, fin d'abonnement en filet), page de réglages avec aperçu en direct. Périmètre élargi en cours de route (titre par email, signature dédiée aux transactionnels, footer réseaux sociaux) et 10 bugs trouvés et corrigés, dont un vrai bug de fiabilité (emails envoyés en fire-and-forget tués en vol par le runtime serverless Vercel avant de partir). Détail complet dans la section Phase 6 plus bas. **3 sous-étapes restent ouvertes** : 6.7 (beat cadeau de fidélité, reporté), et 2 nouveaux trous de scope découverts en fin de session — 6.8 (email de confirmation de création de compte, actuellement seul l'email générique Supabase Auth est envoyé) et 6.9 (le mail free download reste codé en dur hors du système de branding, `/api/free-download/route.ts`).
>
...
|---|---|---|
| 1 | **11d Phase 6** — Mailing transactionnels + beat cadeau fidélité | Comble un vrai trou produit visible immédiatement : aucun email de confirmation n'est envoyé après un achat ou un abonnement. Indépendant du nom de domaine (réutilise le même mécanisme d'envoi que Campagnes, déjà fonctionnel sur le domaine temporaire). |
| 2 | **11d Phase 7** — Catégories & Certification | Indépendant du nom. Prérequis fonctionnel à l'étape 15 (la modération des demandes de certification fait partie du périmètre Admin — impossible à construire tant que les demandes elles-mêmes n'existent pas). |
...
### Phase 6 — Mailing : Transactionnels 🔄 En cours (6.1-6.6 validés, reste 6.7-6.9)
...
| 6.4 | Page `/dashboard/business/mailing/transactionnels/` : vue scindée avec aperçu en direct (anti-rafale 400ms), branding partagé (couleur, signature dédiée, footer réseaux avec titre+phrase) + titre et intro personnalisables par type | ✅ |
...
| 6.8 | **Email officiel de confirmation de création de compte** (trou de scope découvert le 2026-07-17) — aujourd'hui `/artiste/inscription` ne déclenche que l'email générique de confirmation Supabase Auth, jamais un email de bienvenue brandé | ⬜ |
| 6.9 | **Personnalisation du mail free download** (trou de scope découvert le 2026-07-17) — `/api/free-download/route.ts` envoie un HTML codé en dur (couleurs fixes indigo/vert, aucun branding), hors du système `templates_transactionnels`/`rendreEmailTransactionnel` construit en 6.1-6.6 | ⬜ |

> **Bilan 6.1-6.6 (2026-07-17) :** périmètre élargi par rapport au plan d'origine du 2026-07-02 — un 4e type temps réel (`demande_annulation_abonnement`) a été ajouté en cours de route : le plan initial ne notifiait le client qu'à la fin RÉELLE de la période (`customer.subscription.deleted`, parfois des semaines après avoir annulé), sans confirmation immédiate ni date de fin d'accès (retour Jake en testant). `annulation_abonnement` reste comme filet, restreint au cas où aucune demande n'a précédé la suppression. Personnalisation aussi élargie : titre par email (pas seulement l'intro), signature dédiée aux transactionnels (distincte de celle des Automatisations — Jake signe "Jake" en automatisation, "Jake B" en transactionnel), footer réseaux sociaux avec vraies icônes SVG (titre + phrase personnalisables), aperçu en direct plutôt qu'un bouton "Aperçu".
>
> **10 bugs trouvés et corrigés pendant les tests**, dont un vrai bug de fiabilité qui a pris plusieurs cycles à isoler : `demande_annulation_abonnement` ne s'envoyait jamais, sans aucune erreur ni log. Deux causes empilées — (1) `subscription.cancel_at_period_end=true` ne remplit **pas** `subscription.cancel_at` côté Stripe (mécanismes séparés, confirmé par la doc), donc le garde-fou sur ce champ bloquait silencieusement l'envoi ; corrigé en lisant `subscription.items.data[0].current_period_end`. (2) Cause racine réelle : l'envoi était en fire-and-forget (`.catch()` sans `await`) en toute dernière instruction de sa fonction, sans rien après pour laisser à la promesse (appel Resend + écriture `email_logs`) le temps de finir avant que Vercel ne gèle l'instance serverless à la réponse du webhook — silence total garanti. Les 3 autres emails transactionnels avaient plus de travail asynchrone après leur appel, masquant le même risque par chance de timing ; les 4 envois du webhook sont maintenant `await`és. Autres bugs : montant abonnement affiché en centimes au lieu d'euros, largeur d'email trop étroite sur Gmail desktop (600px), page `/artiste/mot-de-passe-oublie` manquante (404), contexte boutique perdu dans la chaîne connexion→mot de passe oublié, lien de reset cassé par un double `?` (Supabase colle `?token_hash=...` à la fin du `redirectTo` sans vérifier s'il contient déjà un `?` — contexte boutique déplacé en paramètre de chemin), build Vercel cassé (`lib/stripe.ts` instanciait le client Stripe au chargement du module), erreurs Stripe non catchées sur annuler/reprendre (message trompeur "Impossible de joindre le serveur"), filet `annulation_abonnement` qui s'envoyait sans condition (doublon avec la demande d'annulation).
